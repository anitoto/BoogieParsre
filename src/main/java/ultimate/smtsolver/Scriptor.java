 ...
 * SMTLIB-2 compliant and expect commands on
 ...
 * return a useful proof object. Also
 * commands, for which the output format is not fully specified, e.g.
 * (get-model), may not return useful return values.
 * 
 * @author Oday Jubran
...
public class Scriptor extends NoopScript {
	 ...
	 * 
	 * @param command
	 *                the command that starts the external SMT solver. The solver
	 *                is expected to read smtlib 2 commands on
	 *                stdin.
	 * @param storage
	 * @throws IOExceptionO
	 *                      If the solver is not installed
	 */
	public Scriptor(final String command, final String solverName, final Logger logger, final Storage storage)
			throws IOException {
		this(command, solverName, false, logger, storage);
	}

	public Scriptor(final String command, final String solverName, boolean omitLogicOutput, final Logger logger,
