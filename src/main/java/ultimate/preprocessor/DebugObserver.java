...
public class DebugObserver extends BaseObserver {
	...
	public DebugObserver(final Logger logger) {
		this.logger = logger;
	}
	...
	private void walkTree(final IWalkable root) {
		...
		while (!queue.isEmpty()) {
			...
			if (!processed.add(node)) {
				logger.fine("This is not a tree! " + node + " is on a cylce");
				continue;
			}
			checkNode(node);
			...
			for (final IWalkable succ : node.getSuccessors()) {
				if (succ == null) {
					logger.fine(node + " contains null successors");
					continue;
				}
				queue.add(succ);
			}
