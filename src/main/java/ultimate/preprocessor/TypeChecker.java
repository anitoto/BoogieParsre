...
import ultimate.ast.BreakStatement;
...
import ultimate.ast.LoopInvariantSpecification;
import ultimate.ast.ModifiesSpecification;
import ultimate.ast.NamedAttribute;
import ultimate.ast.ParentEdge;
...
import ultimate.models.observers.BaseObserver;
import ultimate.type.BoogieFunctionSignature;
import ultimate.type.BoogieStructType;
import ultimate.type.BoogieType;
import ultimate.typechecker.FunctionInfo;
import ultimate.typechecker.ProcedureInfo;
import ultimate.typechecker.TypeCheckHelper;
import ultimate.typechecker.TypeManager;
import ultimate.typechecker.TypeParameters;
import ultimate.typechecker.VariableInfo;
...
public class TypeChecker extends BaseObserver {
	...
	/**
	 * Maps a procedure identifier to all variables that occur in a modifies clause
	 * of this procedure.
	 */
	...
	public TypeChecker(final Logger logger) {
		this.logger = logger;
		mCache = new HashMap<>();
	}
		...
		if (resultType == null) {
			...
			if (expr instanceof BinaryExpression) {
				...
				resultType = TypeCheckHelper.typeCheckUnaryExpression(unexp.getOperator(),
						typecheckExpression(unexp.getExpr()), expr, logger);
				...
				resultType = TypeCheckHelper.typeCheckBitVectorAccessExpression(TypeCheckHelper.getBitVecLength(bvType),
						bvaexpr.getEnd(), bvaexpr.getStart(), bvType, expr, logger);
				...
				resultType = TypeCheckHelper.typeCheckStructAccessExpressionOrLhs(
						typecheckExpression(sae.getStruct()).getUnderlyingType(), sae.getField(), expr.toString(),
						logger);
				...
				resultType = TypeCheckHelper.typeCheckIfThenElseExpression(condType, left, right, ite.getCondition(),
						logger);
	...
	/**
	 * Compare existingDeclInfo with correctDeclInfo and raise an internalError if
	...
	private BoogieType typecheckLeftHandSide(final LeftHandSide lhs) {
		...
		if (lhs instanceof VariableLHS) {
			...
			resultType = TypeCheckHelper.typeCheckArrayAccessExpressionOrLhs(arrayType, indicesTypes, lhs.toString(),
					logger);
		} else {
	...
	public void processProcedureDeclaration(final Procedure proc) {
		...
		final ProcedureInfo pi = new ProcedureInfo(proc, typeParams,
	...
	private void typecheckStatement(final Stack<String> outer, final HashSet<String> allLabels,
		...
		if (statement instanceof AssumeStatement) {
			...
			TypeCheckHelper.typeCheckAssignStatement(lhsIds, lhsTypes, rhsTypes, statement.toString(), logger);
	...
	/**
	 * Check if it is legal to modify variable var and if the variable was declared
	 * at all. It is not legal to modify an
	 * in-parameter of a procedure. It is not legal to modify an global variable
	 * that does not appear in the modifies
	 * clause of the procedure.
	 *
	 * @param lhs
	 *            location of the checked variable
	 * @return BoogieType of the checked variable. errorType if the variable was not
	 *         declared.
	 */
	private BoogieType checkVarModification(final BoogieASTNode BoogieASTNode, final String var) {
		if (mInParams.contains(var)) {
			...
			final String message = "Variable " + var + " modified in procedure " + mCurrentProcedure
					+ " but not declared";
			typeError(BoogieASTNode, message);
	...
	/**
	 * Check if each modified variable of the called procedure is in the modifies
	 * clause of the current procedure.
	 */
	...
	/**
	 * Check if identifier id was already used in the definition of an in parameter,
	 * out parameter of local variable.
	 */
	...
	public boolean process(final IElement root) {
		if (root instanceof Unit) {
			...
			mTypeManager = new TypeManager(unit.getDeclarations(), logger);
			mTypeManager.init();
	...
	private void typeError(final BoogieASTNode BoogieASTNode, final String message) {
		logger.severe(BoogieASTNode.getLocation() + ": " + message);
	}
}
