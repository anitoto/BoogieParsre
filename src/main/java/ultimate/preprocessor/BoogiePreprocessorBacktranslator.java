...
import ultimate.models.annotation.ConditionAnnotation;
...
import ultimate.type.BoogieConstructedType;
...
public class BoogiePreprocessorBacktranslator
	...
	private final boolean emit_backtranslation_warnings;
	/**
	...
	BoogiePreprocessorBacktranslator(
			final Logger logger,
		...
		this.logger = logger;
		this.emit_backtranslation_warnings = emit_backtranslation_warnings;
	...
	void addMapping(final BoogieASTNode inputNode, final BoogieASTNode outputNode) {
		...
		logger.fine("Create mapping between");
		logger.fine("\tOutput " + printDebug(outputNode));
		logger.fine("\tInput  " + printDebug(realInputNode));
	}
	...
	public IProgramExecution<BoogieASTNode, Expression> translateProgramExecution(
		...
		final ProgramState<Expression> newInitialState = translateProgramState(
				programExecution.getInitialProgramState());
		...
		if (newElem == null) {
			if (elem instanceof EnsuresSpecification) {
				...
				if (formula instanceof BooleanLiteral && ((BooleanLiteral) formula).getValue()) {
					...
					// does not provide any additional information. We exclude it from the error
					// path.
					return null;
				}
	...
	private IProgramExecution<BoogieASTNode, Expression> createProgramExecutionFromTrace(
		...
		assert checkCallStackTarget(actualAtomicTrace, logger) : "callstack broke during translation by "
	...
	private boolean checkProcedureNames(final BoogieASTNode elem, final AtomicTraceElement<BoogieASTNode> ate) {
		if (elem instanceof CallStatement) {
			...
			if (!rtr) {
				logger.severe("Call stack broken at " + ate);
				return false;
			}
			return true;
	...
	private static boolean checkProcedureNames(final CallStatement elem, final AtomicTraceElement<BoogieASTNode> ate) {
		if (ate.hasThreadId()) {
			// TODO: is there a correspondence between procedure names and call statements
	...
	public IBacktranslatedCFG<String, BoogieASTNode> translateCFG(final IBacktranslatedCFG<String, BoogieASTNode> cfg) {
		// final IFunction<Map<IExplicitEdgesMultigraph<?, ?, ?, BoogieASTNode, ?>,
		// Multigraph<?, BoogieASTNode>>,
		// IMultigraphEdge<?, ?, ?, BoogieASTNode, ?>, Multigraph<?, BoogieASTNode>,
		// Multigraph<?, BoogieASTNode>>
		// funTranslateEdge =
	...
	private void reportUnfinishedBacktranslation(final String message) {
		if (!emit_backtranslation_warnings) {
			return;
		}
		logger.warning("Unfinished Backtranslation: " + message);
	}

	@Override
	protected void printBrokenCallStackSource(final List<AtomicTraceElement<BoogieASTNode>> trace, final int i) {
		logger.severe(new ProgramExecutionFormatter<>(new BoogieBacktranslationValueProvider())
	...
	private class ExpressionTranslator extends BoogieTransformer {
		...
		protected Expression processExpression(final Expression expr) {
			...
			if (!(expr instanceof IdentifierExpression)) {
				// we only have to translate identifier expressions; so we just descend (using
				// BoogieTransformer)
			...
			if (type instanceof BoogieStructType) {
				...
				if (ct.equals(ct.getUnderlyingType())) {
					...
					if (ct.getClass() != ct.getUnderlyingType().getClass()) {
						// Boogie types have a strange equals implementation, so we need to recheck the
						// underlying type
