...
import ultimate.logic.MutableRational;
import ultimate.logic.Rational;
import ultimate.logic.Term;
import ultimate.logic.Theory;
import ultimate.smtinterpol.Config;
import ultimate.smtinterpol.LogProxy;
import ultimate.smtinterpol.convert.EqualityProxy;
import ultimate.smtinterpol.convert.SharedTerm;
import ultimate.smtinterpol.dpll.Clause;
import ultimate.smtinterpol.dpll.DPLLAtom;
import ultimate.smtinterpol.dpll.DPLLEngine;
...
import ultimate.smtinterpol.proof.LeafNode;
import ultimate.smtinterpol.theory.cclosure.CCEquality;
import ultimate.smtinterpol.util.ArrayMap;
...
public class LinArSolve implements ITheory {
	...
	public LinVar generateLinVar(final TreeMap<LinVar, Rational> factors) {
		...
		if (var == null) {
			...
			final ArrayMap<LinVar, BigInteger> intSum = new ArrayMap<LinVar, BigInteger>(vars, coeffs);
	...
	public Clause computeConflictClause() {
		...
		assert checkClean();
		mEngine.getLogger().debug("cong: " + cong);
		for (final LinVar v : mLinvars) {
			...
			if (lit != null) {
				...
				mEngine.getLogger().debug("Using " + lit + " to ensure disequality " + ea.negate());
			}
		}
	...
	private Clause setBound(LAReason reason) {
		...
		if (reason.isUpper()) {
			...
			for (final LAEquality laeq : var.mEqualities.subMap(bound.add(var.getEpsilon()),
	...
	private void prepareModel() {
		...
		final Map<Rational, Set<ExactInfinitNumber>> sharedPoints = new TreeMap<Rational, Set<ExactInfinitNumber>>();
	...
	public void dumpModel(final LogProxy logger) {
		...
		for (final LinVar var : mLinvars) {
			if (!var.isInitiallyBasic()) {
				logger.debug(var + " = " + realValue(var));
			}
		}
	}
	...
	public void printStatistics(final LogProxy logger) {
		logger.debug("Number of Bland pivoting-Operations: "
				+ mNumPivotsBland + "/" + mNumPivots);
		logger.debug("Number of switches to Bland's Rule: " + mNumSwitchToBland);
		int basicVars = 0;
		for (final LinVar var : mLinvars) {
			if (!var.isInitiallyBasic()) {
				basicVars++;
		...
		logger.debug("Number of variables: " + mLinvars.size()
				+ " nonbasic: " + basicVars + " shared: " + mSharedVars.size());
		logger.debug("Time for pivoting         : " + mPivotTime / 1000000);// NOCHECKSTYLE
		logger.debug("Time for bound computation: " + mPropBoundTime / 1000000);// NOCHECKSTYLE
		logger.debug("Time for bound setting    : " + mPropBoundSetTime / 1000000);// NOCHECKSTYLE
		logger.debug("Time for bound comp(back) : " + mBacktrackPropTime / 1000000);// NOCHECKSTYLE
		logger.debug("Composite::createLit: " + mCompositeCreateLit);
		logger.debug("Number of cuts: " + mNumCuts);
		logger.debug("Time for cut-generation: " + mCutGenTime / 1000000);// NOCHECKSTYLE
		logger.debug("Number of branchings: " + mNumBranches);
	}
	...
	private Clause fixOobs() {
		...
		poll_loop: while (!mOob.isEmpty()) {
			...
			do {
				...
				assert (entry == oob.mHeadEntry
						|| entry.mColumn.getValue().compareTo(entry.mColumn.getUpperBound()) <= 0);
				assert (entry == oob.mHeadEntry
						|| entry.mColumn.getValue().compareTo(entry.mColumn.getLowerBound()) >= 0);
	...
	private Clause propagateBound(final LinVar basic, boolean isUpper) {
		...
		if (isUpper ? bound.less(basic.getUpperBound())
			...
			final CompositeReason newComposite = new CompositeReason(basic, bound, isUpper,
	...
	private void mutate() {
		...
		for (final LinVar lv : mLinvars) {
			...
			for (MatrixEntry it1 = lv.mHeadEntry.mNextInCol; it1 != lv.mHeadEntry; it1 = it1.mNextInCol) {
				...
				if (basic.mDisequalities != null) {
					for (final Rational diseq : basic.mDisequalities.keySet()) {
						...
						final ExactInfinitNumber bad = new ExactInfinitNumber(diseq, Rational.ZERO)
			...
			final InfinitNumber chosen = choose(prohib, sharedPoints, lcm, lv.getValue());
	...
	Map<ExactInfinitNumber, List<SharedTerm>> getSharedCongruences() {
		...
		final Map<ExactInfinitNumber, List<SharedTerm>> result = new HashMap<ExactInfinitNumber, List<SharedTerm>>();
	...
	private Clause mbtc(final Map<ExactInfinitNumber, List<SharedTerm>> cong) {
		for (final Map.Entry<ExactInfinitNumber, List<SharedTerm>> congclass : cong.entrySet()) {
			...
			mEngine.getLogger().debug("propagating MBTC: " + lcongclass);
			...
			while (it.hasNext()) {
				...
				if (cceq.getLASharedData().getDecideStatus() != null) { // NOPMD
					if (cceq.getDecideStatus() == cceq.negate()) {
						...
						mEngine.getLogger().debug("already set: " + cceq.getAtom().getDecideStatus());
					}
				} else {
					mEngine.getLogger().debug("MBTC: Suggesting literal " + cceq);
		...
		for (final LinVar v : mLinvars) {
			if (v.mBasic) {
				...
				if (v.getValue().mEps > 0) {
					...
					final InfinitNumber diff = new InfinitNumber(v.getValue().mA, 0).sub(v.getLowerBound());
					if (diff.compareTo(maxeps) < 0) {
						maxeps = diff;
					}
					assert (v.getValue().mEps == -1);
	...
	public LAEquality createEquality(final MutableAffinTerm at) {
		...
		if (sharedData == null) {
			sharedData = new LAEquality(mEngine.getAssertionStackLevel(), var, bound.mA);
	...
	public Literal createCompositeLiteral(final LAReason comp, final Literal beforeLit) {
		...
		final InfinitNumber litBound = comp.isUpper() ? bc.getBound() : bc.getInverseBound();
