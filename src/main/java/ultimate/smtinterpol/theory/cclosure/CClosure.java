...
import ultimate.smtinterpol.dpll.SimpleList;
...
import ultimate.smtinterpol.theory.cclosure.CCTermPairHash.Info.Entry;
import ultimate.smtinterpol.theory.linar.EQAnnotation;
import ultimate.smtinterpol.theory.linar.LAEquality;
import ultimate.smtinterpol.util.ArrayQueue;
...
public class CClosure implements ITheory {
	...
	final ScopedHashMap<Object, CCBaseTerm> mSymbolicTerms = new ScopedHashMap<Object, CCBaseTerm>();
	int mNumFunctionPositions;
	int mMergeDepth;
	final ArrayDeque<CCTerm> mMerges = new ArrayDeque<CCTerm>();
	final ArrayDeque<SymmetricPair<CCAppTerm>> mPendingCongruences = new ArrayDeque<SymmetricPair<CCAppTerm>>();
	...
	public void dumpModel(LogProxy logger) {
		...
		logger.debug("Equivalence Classes:");
	...
	public void printStatistics(LogProxy logger) {
		logger.debug("CCTimes: iE " + mInvertEdgeTime + " eq "
				+ mEqTime + " cc " + mCcTime + " setRep "
				+ mSetRepTime);
		logger.debug("Merges: " + mMergeCount + ", cc:" + mCcCount);
	}
	...
	private Clause buildCongruence(boolean checked) {
		...
		while ((cong = mPendingCongruences.poll()) != null) {
			mEngine.getLogger().debug("PC " + cong);
			Clause res = null;
