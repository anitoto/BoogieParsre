 ...
 */
package ultimate.smtinterpol.smtlib2;
...
import ultimate.logic.CheckClosedTerm;
import ultimate.logic.ConstantTerm;
import ultimate.logic.FormulaUnLet;
import ultimate.logic.FunctionSymbol;
import ultimate.logic.FunctionSymbolFactory;
import ultimate.logic.Logics;
import ultimate.logic.Model;
import ultimate.logic.NoopScript;
import ultimate.logic.PrintTerm;
import ultimate.logic.QuotedObject;
import ultimate.logic.Rational;
import ultimate.logic.ReasonUnknown;
...
import ultimate.smtinterpol.DefaultLogger;
import ultimate.smtinterpol.LogProxy;
import ultimate.smtinterpol.Main;
...
import ultimate.smtinterpol.interpolate.Interpolator;
import ultimate.smtinterpol.interpolate.SymbolChecker;
import ultimate.smtinterpol.interpolate.SymbolCollector;
import ultimate.smtinterpol.option.OptionMap;
import ultimate.smtinterpol.option.OptionMap.CopyMode;
import ultimate.smtinterpol.option.SolverOptions;
import ultimate.smtinterpol.proof.ProofChecker;
import ultimate.smtinterpol.proof.ProofTermGenerator;
import ultimate.smtinterpol.proof.PropProofChecker;
import ultimate.smtinterpol.proof.SourceAnnotation;
import ultimate.smtinterpol.proof.UnsatCoreCollector;
import ultimate.util.datastructures.ScopedArrayList;

/**
 * Implementation of the {@link ultimate.logic.Script} interface to interact with
 * SMTInterpol.
 *
 * Users should however stick to the {@link ultimate.logic.Script} interface which provides
...
public class SMTInterpol extends NoopScript {
	...
	ultimate.smtinterpol.model.Model mModel = null;
	...
	public LBool checkSatAssuming(final Term... assumptions) throws SMTLIBException {
		...
		if (mSolverOptions.getCheckType().check(mEngine)) {
			if (mEngine.hasModel()) {
				...
				if (mSolverOptions.isModelCheckModeActive()) {
					mModel = new ultimate.smtinterpol.model.Model(mClausifier, getTheory(),
					...
					for (final Term asserted : mAssertions) {
						...
						if (checkedResult != getTheory().mTrue) {
							...
							mLogger.severe("Model does not satisfy " + asserted.toStringDirect());
			...
			if (mSolverOptions.isProofCheckModeActive()) {
				...
				if (!proofchecker.check(getProof())) {
					...
					mLogger.severe("Proof-checker did not verify");
		...
		if (Config.CHECK_STATUS_SET && isStatusSet() && mReasonUnknown != ReasonUnknown.MEMOUT
			...
			mLogger.warning("Status differs: User said %s but we got %s", mStatusSet, mStatus);
			if (mDDFriendly) {
				System.exit(13);
			}
		}
	...
	public Term[] getInterpolants(final Term[] partition, final int[] startOfSubtree) {
		...
		try {
			...
			if (mSolverOptions.isInterpolantCheckModeActive()) {
				...
				try {
					tmpBench.mLogger.setLoglevel(LogProxy.LOGLEVEL_SEVERE);
				...
				try {
					...
					for (int i = 0; i < startOfSubtree.length; ++i) {
						...
						try {
							...
							mLogger.severe("Could not assert interpolant", exc);
							error = true;
						}
						...
						if (res == LBool.SAT) {
							...
							mLogger.severe("Interpolant %d not inductive: " + " (Check returned %s)", i, res);
							error = true;
						} else if (res == LBool.UNKNOWN) {
							final ReasonUnknown ru = tmpBench.mReasonUnknown;
							mLogger.warning("Unable to check validity of interpolant: " + ru);
		...
		if (getBooleanOption(":unsat-core-check-mode")) {
			...
			try {
				...
				if (isUnsat != LBool.UNSAT) {
					mLogger.severe("Unsat core could not be proven unsat (Result is %s)", isUnsat);
				}
			} finally {
				tmpBench.mLogger.setLoglevel(old);
				// Not needed for now, but maybe later...
	...
	public Term[] getUnsatAssumptions() throws SMTLIBException, UnsupportedOperationException {
		...
		if (getBooleanOption(":unsat-assumptions-check-mode")) {
			...
			try {
				...
				final LBool isUnsat = tmpBench.checkSat();
				if (isUnsat != LBool.UNSAT) {
					mLogger.severe("Unsat assumptions could not be proven unsat (Result is %s)", isUnsat);
				}
			} finally {
				tmpBench.mLogger.setLoglevel(old);
				// Not needed for now, but maybe later...
