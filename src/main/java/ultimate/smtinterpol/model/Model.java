 ...
 */
package ultimate.smtinterpol.model;
...
import ultimate.smtinterpol.model.FunctionValue.Index;
 ...
 */
public class Model implements ultimate.logic.Model {
			...
			if (fs.getReturnSort().getName().equals("Int")) {
				if (!mNumSorts.get(me.getValue().getDefault()).isIntegral()) {
					...
					if (fs.getParameterSorts().length == 0) {
						logger.severe("Non-integral value for integer variable " + fs);
					} else {
						logger.severe("Non-integral default value for function " + fs);
					}
				}
				for (final Map.Entry<Index, Integer> val : me.getValue().values().entrySet()) {
					if (!mNumSorts.get(val.getValue()).isIntegral()) {
						correct = false;
						logger.severe("Non-integral value for function " + fs + " at index " + me.getKey());
					}
				}
			}
		}
