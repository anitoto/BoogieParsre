...
import ultimate.type.BoogieTypeConstructor;
...
public class TypeManager {
	...
	public TypeManager(final Declaration[] decls, final Logger logger) {
		this.logger = logger;
	...
	public BoogieType getPrimitiveType(final String typeName) {
		if (typeName.equals("int")) {
			...
			logger.severe("getPrimitiveType called with unknown type " + typeName + "!");
	...
	public BoogieType resolveNamedType(final NamedType type, final boolean markUsed) {
		...
		while (it.hasPrevious()) {
			...
			if (placeholderType != null) {
				if (numParam != 0) {
					logger.severe("Bounded type " + name + " used with arguments.");
					return BoogieType.TYPE_ERROR;
				}
		...
		if (!typeConstructors.containsKey(name)) {
			...
			if (decl == null) {
				logger.severe("Type " + name + " is never defined.");
				return BoogieType.TYPE_ERROR;
			}
			resolve(decl);
		}
		...
		if (tc.getParamCount() != numParam) {
			logger.severe("Type " + name + " used with wrong number of arguments.");
			return BoogieType.TYPE_ERROR;
		}
		...
		for (int i = 0; i < numParam; i++) {
			/*
			 * Resolve the other type arguments without marking place holders as used. Place
			 * holders are actually
	...
	public BoogieType resolveArrayType(final ArrayType type, final boolean markUsed) {
		...
		if (!typeParams.fullyUsed()) {
			logger.severe("ArrayType generics not used in index types: " + type);
		}
	...
	public BoogieType resolveType(final ASTType type, final boolean markUsed) {
		...
		if (type instanceof PrimitiveType) {
			...
			logger.severe("Unknown ASTType " + type);
	...
	public void resolve(final TypeDeclaration td) {
		if (visiting.contains(td.getIdentifier())) {
			logger.severe("Cyclic type definition: " + visiting);
