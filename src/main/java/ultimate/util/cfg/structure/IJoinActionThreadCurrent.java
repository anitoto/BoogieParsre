...
package ultimate.util.cfg.structure;
 ...
 * <ul>
 * <li>variables that are assigned by the fork get the values that are returned
 * by the procedure
 * <li>all local variables that occure only in the context of the forked
 * procedure are havoced.
 * <ul/>
 *
...
public interface IJoinActionThreadCurrent extends IAction {
	...
	public static class JoinSmtArguments {
		...
		public UnmodifiableTransFormula constructThreadIdAssumption(final IIcfgSymbolTable symbolTable,
				final ManagedScript mgdScript, final List<IProgramVar> leftHandSides, final Logger logger) {
		...
		public UnmodifiableTransFormula constructResultAssignment(final ManagedScript mgdScript,
				final List<? extends IProgramVar> rightHandSides, final Logger logger) {
			final List<IProgramVar> assignmentLhs = getAssignmentLhs();
			return TransFormulaBuilder.constructAssignment(assignmentLhs, rightHandSides, mgdScript, logger);
		}
	}
