...
/**
 * An object of this class allows one to construct a
 * {@link UnmodifiableTransFormula}. {@link UnmodifiableTransFormula}s
 * are unmodifiable and have a package-private constructor. This class allows to
 * collect data for a TransFormula and to
 * construct it.
 *
...
public class TransFormulaBuilder {
	...
	/**
	 * Specify inVars, outVars, auxVars, and branchEncoders that are used initially
	 * while constructing a new
	 * {@link UnmodifiableTransFormula}. For each of these arguments we do not use
	 * the Map/Set but construct a copy.
	 * Each of these arguments my be null, and if this is the case we start with an
	 * empty Map/Set. If emptyAuxVars or
	 * emptyBranchEncoders is "true" we use an emptyMap/emptySet for the respective
	 * Map/Set.
	 */
	public TransFormulaBuilder(final Map<IProgramVar, TermVariable> inVars,
			...
			final Collection<TermVariable> branchEncoders, final boolean emptyAuxVars, final Logger logger) {
		super();
		this.logger = logger;
		if (inVars == null) {
			mInVars = new HashMap<>();
	...
	/**
	 * Add a set of aux vars but rename each of them to a fresh constant. Requires
	 * that the formula is already set,
	...
	public void addAuxVarsButRenameToFreshCopies(final Set<? extends TermVariable> arg0, final ManagedScript script) {
		...
		mFormula = new Substitution(script, substitutionMapping, logger).transform(mFormula);
	}
	...
	public UnmodifiableTransFormula finishConstruction(final ManagedScript script) {
		...
		return new UnmodifiableTransFormula(mFormula, mInVars, mOutVars, mNonTheoryConsts, mAuxVars, mBranchEncoders,
				mInfeasibility, script, logger);
	}

	/**
	 * Construct TransFormula with "true" formula and no variables.
	 */
	public static UnmodifiableTransFormula getTrivialTransFormula(final ManagedScript script, final Logger logger) {
		final TransFormulaBuilder tfb = new TransFormulaBuilder(null, null, true, null, true, null, true, logger);
	...
	/**
	 * Construct TransFormula that represents the identity relation restricted to
	 * the predicate pred, i.e., if x is the
	 * vector of variables occurring in pred, the result represents a formula
	 * φ(x,x') such that the following holds.
	 * <ul>
	...
	public static UnmodifiableTransFormula constructTransFormulaFromPredicate(final IPredicate pred,
			final ManagedScript script, final Logger logger) {
		final Set<ApplicationTerm> consts = new ConstantFinder().findConstants(pred.getFormula(), false, logger);
		if (!consts.isEmpty()) {
		...
		for (final IProgramVar bv : pred.getVars()) {
			final TermVariable freshTv = script.constructFreshTermVariable(bv.getGloballyUniqueId(),
		...
		final Term formula = new Substitution(script.getScript(), substitutionMapping, logger).transform(pred.getFormula());
	 ...
	 * rhs_1,...,rhs_n, construct a {@link TransFormula}
	...
	public static UnmodifiableTransFormula constructAssignment(final List<? extends IProgramVar> lhs,
			...
			final Logger logger) {
		return constructEquality(lhs, rhs, symbolTable, mgdScript, false, logger);
	}
	...
	public static UnmodifiableTransFormula constructEqualityAssumption(final List<? extends IProgramVar> lhs,
			final List<Term> rhs, final IIcfgSymbolTable symbolTable, final ManagedScript mgdScript,
			final Logger logger) {
		return constructEquality(lhs, rhs, symbolTable, mgdScript, true, logger);
	}

	private static UnmodifiableTransFormula constructEquality(final List<? extends IProgramVar> lhs,
			final List<Term> rhs, final IIcfgSymbolTable symbolTable, final ManagedScript mgdScript,
			final boolean lhsAreAlsoInVars, final Logger logger) {
		...
		for (int i = 0; i < rhs.size(); i++) {
			final Set<ApplicationTerm> consts = new ConstantFinder().findConstants(rhs.get(i), false, logger);
			if (!consts.isEmpty()) {
				throw new UnsupportedOperationException("constants not yet supported");
			}

			final TermVarsProc tvp = TermVarsProc.computeTermVarsProc(rhs.get(i), mgdScript.getScript(), symbolTable, logger);
		...
		final Substitution subst = new Substitution(mgdScript.getScript(), substitutionMapping, logger);
	...
	/**
	 * Given two list of variables lhs_1,...,lhs_n and rhs_1,...,rhs_n, construct a
	 * {@link TransFormula}
	 * that represents the assignment lhs_1,...,lhs_n := rhs_1,...,rhs_n
	 */
	public static UnmodifiableTransFormula constructAssignment(final List<? extends IProgramVar> lhs,
			final List<? extends IProgramVar> rhs, final ManagedScript mgdScript, final Logger logger) {
		if (lhs.size() != rhs.size()) {
			throw new IllegalArgumentException("different number of argument on LHS and RHS");
		}
	...
	/**
	 * Construct copy of a given {@link Transformula} with minor modifications that
	 * are specified by the arguments of
	 * this method.
	 *
	 * @param script
	 *                          {@link ManagedScript} that was used to construct the
	 *                          {@link Term}s in the input {@link Transformula}
	 * @param tf
	 *                          input {@link Transformula}
	 * @param inVarsToRemove
	 *                          {@link IProgramVar}s whose inVars are removed in the
	 *                          result. If the inVar instance is not also an
	 *                          outVar the {@link TermVariable} becomes an auxVar.
	 * @param outVarsToRemove
	 *                          {@link IProgramVar}s whose outVars are removed in
	 *                          the result. If the outVar instance is not also an
	 *                          inVar the {@link TermVariable} becomes an auxVar.
	 * @param additionalOutVars
	 *                          Map from {@link IProgramVar}s to
	 *                          {@link TermVariable}s that specifies new outVar
	 *                          instances that are
	 *                          added. It is only allowed to add outVars for
	 *                          {@link IProgramVar}s that do not have an outVar (at
	 *                          the
	 *                          point in time where outVars specified in the
	 *                          preceding parameter have been removed).
	 * @return Copy if the input {@link Transformula} whith modifications specified
	 *         by the other parameters of this
	 *         method.
	 */
	public static UnmodifiableTransFormula constructCopy(final ManagedScript script, final TransFormula tf,
			...
			final Map<IProgramVar, TermVariable> additionalOutVars, final Logger logger) {
		...
		final TransFormulaBuilder tfb = new TransFormulaBuilder(tf.getInVars(), tf.getOutVars(),
				...
				branchEncoders.isEmpty(), branchEncoders.isEmpty() ? null : branchEncoders, false, logger);
	...
	public static <E extends IProgramVar> UnmodifiableTransFormula constructCopy(final ManagedScript script,
			final TransFormula tf, final Map<E, E> variableReplacement, final Logger logger) {
		Set<TermVariable> branchEncoders;
		if (tf instanceof UnmodifiableTransFormula) {
			branchEncoders = ((UnmodifiableTransFormula) tf).getBranchEncoders();
		} else {
		...
		final Term newFormula = new Substitution(script, substitutionMapping, logger).transform(tf.getFormula());
