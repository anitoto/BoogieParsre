 ...
 */
package ultimate.util.cfg.transitions;
...
import ultimate.exceptions.RunningTaskInfo;
...
import ultimate.util.cfg.ModifiableGlobalsTable;
import ultimate.util.cfg.transitions.UnmodifiableTransFormula.Infeasibility;
import ultimate.util.cfg.variables.ILocalProgramVar;
import ultimate.util.cfg.variables.IProgramConst;
import ultimate.util.cfg.variables.IProgramNonOldVar;
import ultimate.util.cfg.variables.IProgramOldVar;
import ultimate.util.cfg.variables.IProgramVar;
import ultimate.util.cfg.variables.IProgramVarOrConst;
...
import ultimate.util.smt.ConstantFinder;
import ultimate.util.smt.DagSizePrinter;
import ultimate.util.smt.MonolithicImplicationChecker;
import ultimate.util.smt.PartialQuantifierElimination;
import ultimate.util.smt.SMTPrettyPrinter;
...
import ultimate.util.smt.SubtermPropertyChecker;
...
import ultimate.util.smt.pqe.XnfDer;
import ultimate.util.smt.predicates.BasicPredicateFactory;
import ultimate.util.smt.predicates.IPredicate;
import ultimate.util.smt.predicates.PredicateTransformer;
import ultimate.util.smt.predicates.TermDomainOperationProvider;
...
public final class TransFormulaUtils {
	 ...
	 * transition if it occurs as outVar and - it
	 * does not occur as inVar - or the inVar is represented by a different
	 * TermVariable
	 */
	 ...
	 *                             Apply our partial quantifier elimination and try
	 *                             to eliminate auxVars. This is a postprocessing
	 *                             that
	 *                             we apply to the resulting formula which produces
	 *                             an equivalent formula with less auxvars.
	 * @return the relational composition (concatenation) of transformula1 and
	 *         transformula2
	 */
	public static UnmodifiableTransFormula sequentialComposition(final ManagedScript mgdScript, final boolean simplify,
			...
			final List<UnmodifiableTransFormula> transFormula, final Logger logger, final Storage storage) {
		logger.fine("sequential composition with" + (simplify ? "" : "out") + " formula simplification");
		final Script script = mgdScript.getScript();
		final Set<TermVariable> auxVars = new HashSet<>();
		Term formula = mgdScript.getScript().term("true");

		final TransFormulaBuilder tfb = new TransFormulaBuilder(null, null, false, null, false, null, false, logger);
		...
		for (int i = transFormula.size() - 1; i >= 0; i--) {
			...
			final Term updatedFormula = new SubstitutionWithLocalSimplification(mgdScript, substitutionMapping, logger)
		...
		if (simplify) {
			try {
				final Term simplified = SmtUtils.simplify(mgdScript, formula, simplificationTechnique, logger, storage);
				formula = simplified;
			} catch (final ToolchainCanceledException tce) {
				final String taskDescription = "doing sequential composition of " + transFormula.size()
						+ " TransFormulas";
		...
		if (tryAuxVarElimination) {
			final Term eliminated = PartialQuantifierElimination.elim(mgdScript, QuantifiedFormula.EXISTS, auxVars,
					formula, simplificationTechnique, xnfConversionTechnique, logger, storage);
			logger.fine("DAG size before PQE {0}: " + new DagSizePrinter(formula) + ", DAG size after PQE {1}: "
					+ new DagSizePrinter(eliminated));
			formula = eliminated;
		} else {
			final XnfDer xnfDer = new XnfDer(mgdScript, logger, storage);
			formula = SmtUtils.and(script,
					xnfDer.tryToEliminate(QuantifiedFormula.EXISTS, SmtUtils.getConjuncts(formula), auxVars));
		}
		if (simplify) {
			formula = SmtUtils.simplify(mgdScript, formula, simplificationTechnique, logger, storage);
		} else {
			final LBool isSat = Util.checkSat(script, formula);
			if (isSat == LBool.UNSAT) {
				logger.fine("CodeBlock already infeasible");
		...
		if (tranformToCNF) {
			final Term cnf = SmtUtils.toCnf(mgdScript, formula, xnfConversionTechnique, logger, storage);
			formula = cnf;
		}

		TransFormulaUtils.addConstantsIfInFormula(tfb, formula, nonTheoryConsts, logger);
	 ...
	 * underlying relations. If we check
	 * satisfiability of a path which contains this transFormula we want know one
	 * disjuncts that is satisfiable. We use
	 * additional boolean variables called branchIndicators to encode this
	 * disjunction. Example: Assume we have two
	 * TransFormulas tf1 and tf2. Instead of the Formula tf1 || tf2 we use the
	 * following formula. (BI1 -> tf1) && (BI2
	 * -> tf2) && (BI1 || BI2) The following holds
	 * <ul>
	 * <li>tf1 || tf2 is satisfiable iff (BI1 -> tf1) && (BI2 -> tf2) && (BI1 ||
	 * BI2) is satisfiable.
	 * <li>in a satisfying assignment BIi can only be true if tfi is true for i \in
	 * {1,2}
	 *
	 * @param logger
	 * @param logger
	 ...
	 */
	public static UnmodifiableTransFormula parallelComposition(final Logger logger, final Storage storage,
			final int serialNumber, final ManagedScript mgdScript, final TermVariable[] branchIndicators,
			final boolean tranformToCNF, final XnfConversionTechnique xnfConversionTechnique,
			final UnmodifiableTransFormula... transFormulas) {
		logger.fine("parallel composition");
		...
		if (useBranchEncoders) {
			...
			tfb = new TransFormulaBuilder(null, null, false, null, true, null, false, logger);
		}
		final Set<IProgramConst> nonTheoryConsts = new HashSet<>();
		...
		for (int i = 0; i < transFormulas.length; i++) {
			...
			renamedFormulas[i] = new SubstitutionWithLocalSimplification(mgdScript, substitutionMapping, logger)
					.transform(originalFormula);
		...
		if (tranformToCNF) {
			resultFormula = SmtUtils.toCnf(mgdScript, resultFormula, xnfConversionTechnique, logger, storage);
		}

		TransFormulaUtils.addConstantsIfInFormula(tfb, resultFormula, nonTheoryConsts, logger);
	...
	/**
	 * Returns TransFormula that describes a sequence of code blocks that contains a
	 * pending call. Note the the scope of
	 * inVars and outVars is different. Do not compose the result with the
	 * default/intraprocedural composition.
	 *
	 * @param beforeCall
	 *                                        TransFormula that describes transition
	 *                                        relation before the call.
	 * @param callTf
	 *                                        TransFormula that describes parameter
	 *                                        assignment of call.
	 * @param oldVarsAssignment
	 *                                        TransFormula that assigns to oldVars
	 *                                        of modifiable globals the value of the
	 *                                        global var.
	 * @param globalVarsAssignment
	 *                                        TODO
	 * @param afterCall
	 *                                        TransFormula that describes the
	 *                                        transition relation after the call.
	 * @param logger
	 * @param logger
	 * @param modifiableGlobalsOfEndProcedure
	 *                                        Set of variables that are modifiable
	 *                                        globals in the procedure in which the
	 *                                        afterCall TransFormula
	 *                                        ends.
	 * @param symbolTable
	 */
	public static UnmodifiableTransFormula sequentialCompositionWithPendingCall(final ManagedScript mgdScript,
			...
			final UnmodifiableTransFormula afterCall, final Set<IProgramNonOldVar> modifiableGlobalsOfEndProcedure,
			final XnfConversionTechnique xnfConversionTechnique, final SimplificationTechnique simplificationTechnique,
		...
		final UnmodifiableTransFormula callAndBeforeTF;
		{
			...
			final UnmodifiableTransFormula composition = sequentialComposition(mgdScript, simplify, extPqe,
					transformToCNF, xnfConversionTechnique, simplificationTechnique, callAndBefore, logger, storage);
			...
			for (final IProgramVar bv : composition.getOutVars().keySet()) {
				final boolean isInterfaceVariable = isInterfaceVariable(bv, callTf, oldVarsAssignment, procBeforeCall,
						procAfterCall, true, false);
			...
			final Set<IProgramNonOldVar> modifiableByCaller = modifiableGlobalsTable
					.getModifiedBoogieVars(procBeforeCall);
			...
			// we havoc all local variables of the caller (unless they are inparams of
			// callee)
			...
			callAndBeforeTF = TransFormulaBuilder.constructCopy(mgdScript, composition, Collections.emptySet(),
					outVarsToRemove, varsToHavoc, logger);
		...
		final UnmodifiableTransFormula globalVarAssignAndAfterTF;
		{
			...
			final UnmodifiableTransFormula composition = sequentialComposition(mgdScript, simplify, extPqe,
					transformToCNF, xnfConversionTechnique, simplificationTechnique, oldAssignAndAfterList, logger,
					storage);
		...
		final UnmodifiableTransFormula preliminaryResult = sequentialComposition(mgdScript, simplify, extPqe,
				transformToCNF, xnfConversionTechnique, simplificationTechnique,
				Arrays.asList(new UnmodifiableTransFormula[] { callAndBeforeTF, globalVarAssignAndAfterTF }), logger,
				storage);
		...
		if (procAfterCall.equals(procAtEnd)) {
			...
			if (outVarsToRemove.isEmpty()) {
				...
				result = TransFormulaBuilder.constructCopy(mgdScript, preliminaryResult, Collections.emptySet(),
						outVarsToRemove, Collections.emptyMap(), logger);
			}
		}

		assert !result.getBranchEncoders().isEmpty()
				|| predicateBasedResultCheck(mgdScript, xnfConversionTechnique, simplificationTechnique, beforeCall,
						callTf, oldVarsAssignment, globalVarsAssignment, afterCall, result, symbolTable,
						modifiableGlobalsOfEndProcedure, logger, storage)
				: "sequentialCompositionWithPendingCall - incorrect result";
		return result;
	}

	/**
	 * Check if {@link IProgramVar} is variable at the interface between caller and
	 * callee. This is used for
	 * interprocedural sequential compositions with pending calls. We say that a
	 * variable is an interface variable if it
	 * is either - an inparam of the callee (local variable) - an oldvar that is in
	 * the callee's set of modifiable
	 * variables - an non-old global variable that is not in the callee's set of
	...
	private static boolean predicateBasedResultCheck(final ManagedScript mgdScript,
			final XnfConversionTechnique xnfConversionTechnique, final SimplificationTechnique simplificationTechnique,
			final List<UnmodifiableTransFormula> beforeCall, final UnmodifiableTransFormula callTf,
			final UnmodifiableTransFormula oldVarsAssignment, final UnmodifiableTransFormula globalVarsAssignment,
			final UnmodifiableTransFormula afterCallTf, final UnmodifiableTransFormula result,
			final IIcfgSymbolTable symbolTable, final Set<IProgramNonOldVar> modifiableGlobalsOfEndProcedure,
			final Logger logger, final Storage storage) {
		...
		final PredicateTransformer<Term, IPredicate, TransFormula> pt = new PredicateTransformer<>(mgdScript,
				new TermDomainOperationProvider(mgdScript, logger, storage));
		final BasicPredicateFactory bpf = new BasicPredicateFactory(mgdScript, symbolTable, simplificationTechnique,
		...
		resultComposition = new QuantifierPusher(mgdScript, true, PqeTechniques.ALL_LOCAL, logger, storage)
				.transform(resultComposition);
		...
		endTerm = new QuantifierPusher(mgdScript, true, PqeTechniques.ALL_LOCAL, logger, storage).transform(endTerm);
		final IPredicate endPredicate = bpf.newPredicate(endTerm);
		final MonolithicImplicationChecker mic = new MonolithicImplicationChecker(mgdScript, logger);
	 ...
	 *                                  assignment of call.
	 * @param oldVarsAssignment
	 *                                  TransFormula that assigns to oldVars of
	 *                                  modifiable globals the value of the global
	 *                                  var.
	 * @param procedureTf
	 *                                  TransFormula that describes the procedure.
	 * @param returnTf
	 *                                  TransFormula that assigns the result of the
	 *                                  procedure call.
	 * @param logger
	 * @param logger
	 * @param symbolTable
	...
	public static UnmodifiableTransFormula sequentialCompositionWithCallAndReturn(final ManagedScript mgdScript,
			...
			final UnmodifiableTransFormula returnTf, final XnfConversionTechnique xnfConversionTechnique,
			final SimplificationTechnique simplificationTechnique, final IIcfgSymbolTable symbolTable,
			final Set<IProgramNonOldVar> modifiableGlobalsOfCallee, final Logger logger, final Storage storage) {
		logger.fine("sequential composition (call/return) with" + (simplify ? "" : "out") + " formula simplification");
		final UnmodifiableTransFormula composition = sequentialComposition(mgdScript, simplify, extPqe, transformToCNF,
				xnfConversionTechnique, simplificationTechnique, Arrays.asList(new UnmodifiableTransFormula[] { callTf,
						oldVarsAssignment, globalVarsAssignment, procedureTf, returnTf }),
				logger, storage);
		...
		final UnmodifiableTransFormula result = TransFormulaBuilder.constructCopy(mgdScript, composition,
				inVarsToRemove, outVarsToRemove, additionalOutVars, logger);
		...
		assert !result.getBranchEncoders().isEmpty() || predicateBasedResultCheck(mgdScript, xnfConversionTechnique,
				simplificationTechnique, callTf, oldVarsAssignment, globalVarsAssignment, procedureTf, returnTf, result,
				symbolTable, modifiableGlobalsOfCallee, logger, storage)
	...
	private static boolean predicateBasedResultCheck(final ManagedScript mgdScript,
			...
			final IIcfgSymbolTable symbolTable, final Set<IProgramNonOldVar> modifiableGlobals, final Logger logger,
		...
		Term afterReturnTerm = pt.strongestPostconditionReturn(beforeReturnPredicate, truePredicate, returnTf, callTf,
				oldVarsAssignment, modifiableGlobals);
		afterReturnTerm = new QuantifierPusher(mgdScript, true, PqeTechniques.ALL_LOCAL, logger, storage)
				.transform(afterReturnTerm);
		...
		if (check1 == Validity.UNKNOWN || check2 == Validity.UNKNOWN) {
			logger.warning("predicate-based correctness check returned UNKNOWN, "
	...
	public static UnmodifiableTransFormula computeGuard(final UnmodifiableTransFormula tf,
			final ManagedScript mgdScript, final Logger logger, final Storage storage) {
		...
		final Pair<Term, Set<TermVariable>> termAndAuxVars = tryToEliminateAuxVars(mgdScript, tf.getFormula(), auxVars,
				logger, storage);

		final TransFormulaBuilder tfb = new TransFormulaBuilder(tf.getInVars(), tf.getInVars(),
				tf.getNonTheoryConsts().isEmpty(),
				tf.getNonTheoryConsts().isEmpty() ? null : tf.getNonTheoryConsts(), true, null, false, logger);
	...
	public static UnmodifiableTransFormula computeGuardedHavoc(final UnmodifiableTransFormula tf,
			final ManagedScript mgdScript, final boolean cellPrecisionForArrays, final Logger logger,
			final Storage storage) {
		final Set<TermVariable> auxVars = new HashSet<>(tf.getAuxVars());
		final Map<Term, Term> substitutionMapping = new HashMap<>();
		for (final IProgramVar bv : tf.getAssignedVars()) {
			if (cellPrecisionForArrays && SmtSortUtils.isArraySort(bv.getTermVariable().getSort())) {
				final Set<ApplicationTerm> stores = new ApplicationTermFinder("store", false)
		...
		final Term term = new Substitution(mgdScript, substitutionMapping, logger).transform(tf.getFormula());
		final Pair<Term, Set<TermVariable>> termAndAuxVars = tryToEliminateAuxVars(mgdScript, term, auxVars, logger,
				storage);
	...
	public static UnmodifiableTransFormula negate(final UnmodifiableTransFormula tf, final ManagedScript maScript,
		...
		final Pair<Term, Set<TermVariable>> termAndAuxVars = tryToEliminateAuxVars(maScript, tf.getFormula(),
				tf.getAuxVars(), logger, storage);
		if (!termAndAuxVars.getSecond().isEmpty()) {
			throw new UnsupportedOperationException("cannot negate if there are auxVars");
		}
		final Term formula = SmtUtils.not(maScript.getScript(), termAndAuxVars.getFirst(), logger);

		final TransFormulaBuilder tfb = new TransFormulaBuilder(tf.getInVars(), tf.getOutVars(),
				tf.getNonTheoryConsts().isEmpty(), tf.getNonTheoryConsts().isEmpty() ? null : tf.getNonTheoryConsts(),
				false, tf.getBranchEncoders(), true, logger);
		tfb.setFormula(formula);
	 ...
	 */
	private static Pair<Term, Set<TermVariable>> tryToEliminateAuxVars(final ManagedScript maScript, final Term formula,
			final Set<TermVariable> oldAuxVars, final Logger logger, final Storage storage) {
		final Term resultTerm = PartialQuantifierElimination.quantifier(logger, storage, maScript,
	...
	public static UnmodifiableTransFormula computeMarkhorTransFormula(final UnmodifiableTransFormula tf,
			final ManagedScript maScript, final XnfConversionTechnique xnfConversionTechnique,
			final SimplificationTechnique simplificationTechnique, final Logger logger, final Storage storage) {
		final UnmodifiableTransFormula guard = computeGuard(tf, maScript, logger, storage);
		final UnmodifiableTransFormula negGuard = negate(guard, maScript, xnfConversionTechnique,
				simplificationTechnique, logger, storage);
		final UnmodifiableTransFormula markhor = parallelComposition(logger, storage, tf.hashCode(), maScript, null,
	...
	public static UnmodifiableTransFormula computeEncodedBranchFormula(final UnmodifiableTransFormula tf,
		...
		final UnmodifiableTransFormula blockEnoded = parallelComposition(logger, storage, tf.hashCode(), maScript,
	...
	/**
	 * Add all elements of progConsts to tfb that occur in formula, ignore the those
	 * that do not occur in the formula.
	 */
	public static <T extends IProgramConst> void addConstantsIfInFormula(final TransFormulaBuilder tfb,
			final Term formula, final Set<T> progConsts, final Logger logger) {
		final Set<ApplicationTerm> constsInFormula = new ConstantFinder().findConstants(formula, false, logger);
	...
	public static Term renameInvarsToDefaultVars(final TransFormula tf, final ManagedScript mgdScript,
		...
		return (new Substitution(mgdScript, substitutionMapping, logger)).transform(tf.getFormula());
	}
	...
	public static UnmodifiableTransFormula constructHavoc(final TransFormula tf, final ManagedScript mgdScript,
		...
		final TransFormulaBuilder tfb = new TransFormulaBuilder(tf.getInVars(), tf.getOutVars(), false,
				tf.getNonTheoryConsts(), true, null, false, logger);
	...
	/**
	 * This method first computes the guards of the input
	 * {@link UnmodifiableTransFormula}s. It then returns a
	 * {@link UnmodifiableTransFormula} that is satisfied for some input variables
	 * iff none of the guards is satisfied.
	 * The output variables of the result are simply set to the input variables.
	 *
	 * This method will not always work: in case the disjunction contains auxvars
	 * that cannot be eliminated this method
	 ...
	 * @param logger
	 * @param serialNumber
	 *                      Use a different number in each call. This is an ugly
	 ...
	 */
	public static UnmodifiableTransFormula constructRemainderGuard(final Logger logger, final Storage storage,
			final int serialNumber, final ManagedScript mgdScript, final UnmodifiableTransFormula... transFormulas) {
		final UnmodifiableTransFormula disjunction = parallelComposition(logger, storage, serialNumber, mgdScript,
				null, false, XnfConversionTechnique.BOTTOM_UP_WITH_LOCAL_SIMPLIFICATION, transFormulas);
		final UnmodifiableTransFormula guardOfDisjunction = computeGuard(disjunction, mgdScript, logger, storage);
		return negate(guardOfDisjunction, mgdScript, XnfConversionTechnique.BOTTOM_UP_WITH_LOCAL_SIMPLIFICATION,
				SimplificationTechnique.SIMPLIFY_DDA, logger, storage);
	}

	/**
	 * Substitutes TermVariables in the given TransFormula by other given
	 * TermVariables.
	 *
	 * @param tf
	 *                  The TransFormula
	 ...
	 *                  A ManagedScript
	 * @param mapping
	 *                  A map from the to be replaced variables to their
	 *                  replacements.
	 * @return A new UnmodifiableTransFormula where the variables have been
	 *         substituted.
	 */
	public static UnmodifiableTransFormula substituteTermVars(final TransFormula tf, final ManagedScript mgdScript,
			final Map<TermVariable, TermVariable> mapping, final Logger logger) {
	...
	/**
	 * Attempts to find the IProgramVarOrConst that corresponds to the given term in
	 * the given TransFormula.
	 *
	 * @author Alexander Nutz (nutz@informatik.uni-freiburg.de)
	 *
	 * @param tf
	 * @param term
	 * @return The IProgramVarOrConst that corresponds to term in tf. Null if there
	 *         is none.
	 */
	...
	/**
	 * Attempts to find the IProgramVar that corresponds to the given term in the
	 * given TransFormula.
	 *
	 * @author Alexander Nutz (nutz@informatik.uni-freiburg.de)
	 *
	...
	/**
	 * Pretty print a TransFormula by adding some line breaks to its normal
	 * {@link Object#toString()} representation.
	 * Uses some simple heuristics like "align equality constraints which are
	 * argument to the same and/or by the same
	 * indentation".
	 *
	 * @param tf
	 * @return
