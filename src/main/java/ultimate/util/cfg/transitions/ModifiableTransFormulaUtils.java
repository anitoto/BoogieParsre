...
import ultimate.util.boogie.Boogie2SmtSymbolTable;
import ultimate.util.cfg.IIcfgSymbolTable;
import ultimate.util.cfg.transformations.IReplacementVar;
import ultimate.util.cfg.transformations.IReplacementVarOrConst;
import ultimate.util.cfg.transformations.ReplacementVarFactory;
import ultimate.util.cfg.transformations.ReplacementVarUtils;
...
public class ModifiableTransFormulaUtils {
	 ...
	 * {@link TermVariable}s</i> of a given
	 * {@link IProgramVar} by the default constant for this {@link IProgramVar}.
	 *
	 * @param rvf
	 *                {@link ReplacementVarFactory} that maps {@link IProgramVar}s
	 *                to the corresponding {@link IProgramVar}
	 * @param symbTab
	 *                {@link Boogie2SmtSymbolTable} that maps {@link TermVariable}
	 *                to {@link IProgramVar}s (for the
	 *                {@link TermVariable}s that are default {@link TermVariable}s
	 *                of {@link IProgramVar}s.
	 * @param tf
	 *                {@link ModifiableTransFormula} whose mapping from
	 *                {@link IProgramVar}s to inVars is used.
	 */
	public static Term renameToDefaultConstants(final Script script, final IIcfgSymbolTable symbTab,
			final ModifiableTransFormula tf, final Term term, final Logger logger) {
		...
		final Term result = (new Substitution(script, substitutionMapping, logger)).transform(term);
		return result;
	}
	...
	public static LBool implies(final ModifiableTransFormula antecedent, final ModifiableTransFormula consequent,
			final Script script, final IIcfgSymbolTable symbTab, final Logger logger) {
		final Term antecentTerm = renameToConstants(script, symbTab, antecedent, logger);
		final Term consequentTerm = renameToConstants(script, symbTab, consequent, logger);
		script.push(1);
		script.assertTerm(antecentTerm);
		script.assertTerm(SmtUtils.not(script, consequentTerm, logger));
		script.assertTerm(getAdditionalEqualities(Arrays.asList(antecedent, consequent), symbTab, script, logger));
	...
	/**
	 * Returns a conjunction of equalities of primed and unprimed constants, for
	 * which the corresponding variables don't
	 * occur in the {@code transformulas} directly, but as free variable in the
	 * definition of another occurring
	 * variable.
	 */
	private static Term getAdditionalEqualities(final List<ModifiableTransFormula> transformulas,
			final IIcfgSymbolTable symbTab, final Script script, final Logger logger) {
		...
		for (final TermVariable var : vars) {
			...
			if (!programVars.contains(boogieVar)) {
				final Term equality = SmtUtils.binaryEquality(script, boogieVar.getDefaultConstant(),
						boogieVar.getPrimedConstant(), logger);
				result.add(equality);
			}
		}
	...
	/**
	 * Rename all to inVars/outVars by default/primed constants (including the
	 * definitions of {@link IReplacementVarOrConst}s.
	 * Quantify auxVars existentially.
	 *
	 * @param services
	 * @param logger
	 */
	private static Term renameToConstants(final Script script, final IIcfgSymbolTable symbTab,
			final ModifiableTransFormula tf, final Logger logger) {
		final Map<Term, Term> substitutionMapping = new HashMap<>();
		for (final Entry<IProgramVar, TermVariable> entry : tf.getInVars().entrySet()) {
			if (entry.getKey() instanceof IReplacementVarOrConst) {
				...
				final Term renamedDefinition = renameToDefaultConstants(script, symbTab, tf, definition, logger);
		...
		for (final Entry<IProgramVar, TermVariable> entry : tf.getOutVars().entrySet()) {
			if (entry.getKey() instanceof IReplacementVarOrConst) {
				final Term definition = ReplacementVarUtils.getDefinition(entry.getKey());
				final Term renamedDefinition = renameToPrimedConstants(script, symbTab, tf, definition, logger);
				substitutionMapping.put(entry.getValue(), renamedDefinition);
			} else {
				final IProgramVar bv = entry.getKey();
		...
		Term result = (new Substitution(script, substitutionMapping, logger)).transform(tf.getFormula());
		result = SmtUtils.and(script, result, constructEqualitiesForCoinciding(script, tf, logger));
		if (!tf.getAuxVars().isEmpty()) {
			logger.warning(tf.getAuxVars().size() + " quantified variables");
	 ...
	 * Return a term in which each TermVarialbe
	 * is substituted by the definition of the RankVar. Throws an
	 * IllegalArgumentException if there occurs term contains
	...
	public static Term translateTermVariablesToDefinitions(final Script script, final ModifiableTransFormula tf,
			final Term term, final Logger logger) {
		...
		return (new Substitution(script, substitutionMapping, logger)).transform(term);
	}

	public static List<Term> translateTermVariablesToDefinitions(final Script script, final ModifiableTransFormula tf,
			final List<Term> terms, final Logger logger) {
		final List<Term> result = new ArrayList<Term>();
		for (final Term term : terms) {
			result.add(translateTermVariablesToDefinitions(script, tf, term, logger));
		}
		return result;
	}
		...
		for (final IProgramVar rv : tf.getInVars().keySet()) {
			if (!(rv instanceof IReplacementVarOrConst)) {
				if (inVarAndOutVarCoincide(rv, tf)) {
					...
					conjuncts.add(
							SmtUtils.binaryEquality(script, bv.getDefaultConstant(), bv.getPrimedConstant(), logger));
				}
			}
		}
	...
	/**
	 * Construct a TransFormulaLR from a TransFormula, adding and translating all
	 * existing in- and outVars in the
	 * process.
	 *
	 * @param inputTf
	 *                the TransFormula
	...
	public static ModifiableTransFormula buildTransFormula(final TransFormula inputTf,
			final ReplacementVarFactory replacementVarFactory, final ManagedScript mgdScript, final Logger logger) {
		final Map<Term, Term> substitutionMapping = new HashMap<>();
		...
		final Term formula = (new Substitution(mgdScript, substitutionMapping, logger).transform(inputTf.getFormula()));
