...
public class UnmodifiableTransFormula extends TransFormula implements Serializable {
	 ...
	 * means that TransFormula could be
	...
	/**
	 * This constructor is package-private use {@link TransFormulaBuilder} to
	 * construct TransFormulas.
	 *
	...
	UnmodifiableTransFormula(final Term formula, final Map<IProgramVar, TermVariable> inVars,
		...
		mClosedFormula = computeClosedFormula(formula, super.getInVars(), super.getOutVars(), super.getAuxVars(),
				script, logger);
	...
	private boolean doConstantConsistencyCheck() {
		...
		final Set<ApplicationTerm> constantsInFormula = new ConstantFinder().findConstants(mFormula, false, logger);
	 ...
	 * </ul>
	 * If formula contained no branch encoders the result is a closed formula (does
	 * not contain free variables)
	 *
	 * @param existingAuxVarConsts
	 *                             if true we assume that the constants for the
	 *                             auxVars already exist, otherwise we construct
	 *                             them
	 *
	 */
	public static Term computeClosedFormula(final Term formula, final Map<IProgramVar, TermVariable> inVars,
		...
		final Term closedTerm = new Substitution(script, substitutionMapping, logger).transform(formula);
		return closedTerm;
	}

	/**
	 * Return the constant (resp. 0-ary function symbol) that represents the inVar
	 * of the {@link IProgramVar} pv in the
	...
	/**
	 * Return the constant (resp. 0-ary function symbol) that represents the outVar
	 * of the {@link IProgramVar} pv in the
	 * closed form of the formula of an {@link UnmodifiableTransFormula}.
	 *
	 */
	...
	/**
	 * Remove inVars, outVars and auxVars that are not necessary. Remove auxVars if
	 * it does not occur in the formula.
	 * Remove inVars if it does not occur in the formula. Remove outVar if it does
	 * not occur in the formula and is also
	 * an inVar (case where the var is not modified). Note that we may not generally
	 * remove outVars that do not occur in
	...
	private static boolean allVarsContainsFreeVars(final Set<TermVariable> allVars, final Term term,
		...
		for (final TermVariable tv : freeVars) {
			if (!allVars.contains(tv)) {
				logger.severe("not in allVars: " + tv);
				result = false;
			}
		}
		return result;
	...
	/**
	 * Returns true iff all constants (ApplicationTerm with zero parameters) that
	 * occur in term are contained in the set
	 * setOfConstants.
	 */
	private static boolean isSupersetOfOccurringConstants(final Set<ApplicationTerm> setOfConstants, final Term term,
			final Logger logger) {
		final Set<ApplicationTerm> constantsInTerm = new ConstantFinder().findConstants(term, false, logger);
	...
	private static boolean freeVarsSubsetInOutAuxBranch(final Term term, final Map<IProgramVar, TermVariable> inVars,
			...
			final Set<TermVariable> branchEncoders, final Logger logger) {
		final Set<TermVariable> freeVars = new HashSet<>(Arrays.asList(term.getFreeVars()));
		boolean result = true;
		for (final TermVariable tv : freeVars) {
			...
			logger.severe("neither in out aux: " + tv);
			result = false;
		}
		return result;
	}
	...
	/**
	 * Returns true if each Term variable in mVars occurs as inVar, outVar, auxVar,
	 * or branchEncoder
	 */
	...
	private static void reportTimeoutResult(final Logger logger) {
