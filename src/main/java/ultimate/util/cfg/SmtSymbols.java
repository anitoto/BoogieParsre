...
import ultimate.util.smt.TermClassifier;
import ultimate.util.smt.TermTransferrer;
import ultimate.util.smt.predicates.BasicPredicate;
import ultimate.util.smt.predicates.IPredicate;

/**
 * {@link SmtSymbols} contains axioms and SMT function symbols created
 * throughout a toolchain.
 *
...
public class SmtSymbols {
	 ...
	 */
	public SmtSymbols(final Script script, final Logger logger) {
		this(script.term("true"), new String[0], Collections.emptyMap(), logger);
	}

	/**
	 * Create a {@link SmtSymbols} instance with axioms defined in a single term
	 * together with the procedures in which
	 * they were defined (if any), and a map from defined function names to
	 * {@link SmtFunctionDefinition} instances were
	 * each entry represents a defined SMT function.
	 */
	public SmtSymbols(final Term axioms, final String[] defininingProcedures,
			final Map<String, SmtFunctionDefinition> smtFun2SmtFunDef, final Logger logger) {
		this(new BasicPredicate(HARDCODED_SERIALNUMBER_FOR_AXIOMS, defininingProcedures, axioms, Collections.emptySet(),
				axioms), smtFun2SmtFunDef, logger);
	}

	/**
	 * Create a {@link SmtSymbols} instance with axioms defined by an
	 * {@link IPredicate} and a map from defined function
	 * names to {@link SmtFunctionDefinition} instances were each entry represents a
	 * defined SMT function.
	 */
	...
	/**
	 * Create a new {@link SmtSymbols} instance with an additional axiom without
	...
	public SmtSymbols addAxiom(final Script script, final Term additionalAxioms) {
		...
		return new SmtSymbols(newAxiomsPred, mSmtFunctions2SmtFunctionDefinitions, logger);
	}

	/**
	 * Create a new {@link SmtSymbols} instance with an additional function
	 * declaration.
	 *
	...
	public SmtSymbols addFunction(final Script script, final SmtFunctionDefinition additionalFunction) {
		...
		return new SmtSymbols(mAxioms, map, logger);
	}

	/**
	 * Define all symbols defined by this SmtSymbols in a fresh script (by asserting
	 * or defining)
	 *
	 * @param script
	 *               the new script.
	...
	public void transferSymbols(final Script script) {
		final TermTransferrer tt = new TermTransferrer(script, logger);
	...
	/**
	 * Apply a {@link TermClassifier} to all axioms and function bodies defined by
	 * this {@link SmtSymbols} instance.
	 */
	...
	/**
	 * Replace all function applications of the supplied term that are contained in
	 * this {@link SmtSymbols} instance
	 * with their bodies.
	 *
	...
	private class SmtFunctionInliner extends TermTransformer {
		...
		public void convertApplicationTerm(final ApplicationTerm appTerm, final Term[] newArgs) {
			...
			setResult(new Substitution(mScript, substitutionMapping, logger).transform(body));
		}
	}
