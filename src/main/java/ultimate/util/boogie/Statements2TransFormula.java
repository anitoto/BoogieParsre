...
import ultimate.util.boogie.Boogie2SMT.ConstOnlyIdentifierTranslator;
...
import ultimate.util.boogie.Expression2Term.SingleTermResult;
 ...
 * the transition relation of the
 * statements as SMT formula.
 *
 * Idea of underlying algorithm: Starts at the end of the statement sequence
 * take current variables as outVars and then
 * computes the inVars by traversing the sequence of statements backwards and
 * computing some kind of weakest
 * precondition.
 *
...
public class Statements2TransFormula {
	...
	/**
	 * Compute Formulas that encode violation of one of the added assert statements.
	 * This feature was used in Evrens old
	 * CFG.
	 */
	...
	/**
	 * Try to replace existential quantification by auxiliary variables. Therefore
	 * we bring all terms in prenex normal
	 * form (PNF). If the first quantifier is ∃ we remove it and add the
	 * corresponding variables as auxiliary variables.
	 * Currently our PNF transformation involves a transformation in negation normal
	 * (NNF), hence this skolemization
	...
	public Statements2TransFormula(final Boogie2SMT boogie2smt, final Expression2Term expression2Term,
			final boolean simplePartialSkolemization, final Logger logger, final Storage storage) {
		super();
		this.logger = logger;
	...
	private void initialize(final String procId) {
		...
		mTransFormulaBuilder = new TransFormulaBuilder(null, null, false, null, true, null, false, logger);
	...
	private UnmodifiableTransFormula constructTransFormula(final boolean simplify, final boolean feasibilityKnown,
		...
		if (simplify) {
			formula = SmtUtils.simplify(mMgdScript, formula, simplicationTechnique, logger, storage);
		...
		TransFormulaUtils.addConstantsIfInFormula(mTransFormulaBuilder, formula,
				mConstOnlyIdentifierTranslator.getNonTheoryConsts(), logger);
	...
	/**
	 * Let assign be a statement of the form v_i:=expr_i Remove v_i from the inVars
	 * (if contained). If necessary v_i is
		...
		for (final Specification spec : procedure.getSpecification()) {
			if (spec instanceof ModifiesSpecification) {
				for (final VariableLHS var : ((ModifiesSpecification) spec).getIdentifiers()) {
					...
					final IProgramVar boogieVar = mBoogie2SmtSymbolTable.getBoogieVar(id,
							var.getDeclarationInformation(), false);
					final IProgramVar boogieOldVar = mBoogie2SmtSymbolTable.getBoogieVar(id,
							var.getDeclarationInformation(), true);
					assert boogieVar != null;
	...
	/**
	 * Remove boogieVars from inVars mapping, if the inVar is not an outVar, add it
	 * to he auxilliary variables auxVar.
	 */
	 ...
	 * This is the current inVar. If this
	 * inVar does not yet exist, we create it. In this case we have to add (bv,tv)
	 * to the outVars if bv is not already
	 * an outvar.
	 */
	...
	/**
	 * Construct fresh TermVariable for BoogieVar bv and add it to inVars. Special
	 * case: If BoogieVar bv is an oldVar we
	 * do not take a fresh TermVariable but the default TermVariable for this
	 * BoogieVar.
	 */
	...
	public class GlobalVarTranslatorWithInOutVarManagement extends IdentifierTranslatorWithInOutVarManagement {
		...
		/**
		 * Translate all variables to the non old global variable, independent of the
		 * context. This feature is not used
	 ...
	 * auxVars. Returns a term that is
	 * equivalent to ∃x_1,...,∃x_n input and remove {x_1,...,x_n} from auxVars. The
	 * set {x_1,...,x_n} is determined by
	...
	private void eliminateAuxVarsViaDer() {
		...
		final XnfDer xnfDer = new XnfDer(mMgdScript, logger, storage);
		mAssumes = SmtUtils.and(mScript,
				xnfDer.tryToEliminate(QuantifiedFormula.EXISTS, SmtUtils.getConjuncts(mAssumes), mAuxVars));
	}

	/**
	 * Try to replace existential quantification by auxiliary variables. Bring input
	 * in prenex normal form (PNF). If the
	 * first quantifier is ∃ we remove it and add the corresponding variables to
	 * auxVars.
	 *
	 * @return input in NNF, possibly in a form where the first quantifier block is
	 *         removed.
	 */
	private Term skolemize(final Term input, final Set<TermVariable> auxVars) {
		final Term nnf = new NnfTransformer(mMgdScript, QuantifierHandling.KEEP, logger).transform(input);
		final Term pnf = new PrenexNormalForm(mMgdScript, logger).transform(nnf);
		final QuantifierSequence qs = new QuantifierSequence(mMgdScript.getScript(), pnf, logger);
		...
		if (qvs.isEmpty() || qvs.get(0).getQuantifier() == QuantifiedFormula.FORALL) {
			...
			for (final TermVariable tv : qvs.get(0).getVariables()) {
				final TermVariable newTv = mMgdScript.constructFreshTermVariable("skolemized_" + tv.getName(),
						tv.getSort());
				substitutionMapping.put(tv, newTv);
				auxVars.add(newTv);
			}
			result = new Substitution(mMgdScript, substitutionMapping, logger).transform(qs.getInnerTerm());
		}
		return result;
	}
	...
	/**
	 * Returns a TransFormula that describes the assignment of arguments to callees
	 * (local) input parameters. The
	 * (local) input parameters of the callee are the only outVars. For each
	 * inParameter we construct a new BoogieVar
	 * which is equivalent to the BoogieVars which were constructed while processing
	 * the callee.
	 */
	...
	/**
	 * Outsourced method for getting the inParamAssignment. For detailed
	 * documentation look at the fork and call
	 * specific function.
	 *
	 * @param callee
	 ...
	 * outVars. For each outParameter and each
	 * left hand side of the call we construct a new BoogieVar which is equivalent
	 * to the BoogieVars of the
	...
	public TranslationResult resultAssignment(final CallStatement st, final String caller,
		...
		int offset = 0;
		final DeclarationInformation declInfo = new DeclarationInformation(StorageClass.IMPLEMENTATION_OUTPARAM,
				callee);
	...
	/**
	 * Construct a TermVariable whose name is given by the BoogieVar bv and and
	 * additional suffix. This TermVariable is
	 * not unified. If you use this method make sure that you do not call it twice
	 * for the same combination of bv and
	 * suffix.
	 */
