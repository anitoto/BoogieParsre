...
import ultimate.util.datastructures.NestedMap2;
...
public final class Term2Expression implements Serializable {
	...
	/**
	 * Use symbol table to translate a SMT function application into a Boogie
	...
	/**
	 * Translate term in case it is a bitvector constant as defined as an extension
	 * of the BV logic.
	 */
	...
	/**
	 * Translate a single select expression to an ArrayAccessExpression. If we have
	 * nested select expressions this leads
	 * to nested ArrayAccessExpressions, hence arrays which do not occur in the
	 * boogie program.
	 */
	 ...
	 * ArrayAccessExpression. (see translateSelect why
	...
	private Expression translate(final QuantifiedFormula term) {
		...
		final QuantifierExpression result = new QuantifierExpression(null, type, isUniversal, typeParams, parameters,
				attributes, subformula);
	...
	private Expression translate(final TermVariable term) {
		...
		if (mTranslateState.getQuantifiedVariables().containsKey(term)) {
			...
			if (pv instanceof LocalBoogieVar) {
				...
				// declInfo);
				// } else {
				// throw new AssertionError("unsupported kind of variable " +
	...
	/**
	 * Represents the state that the result of a call to a translate*(Term) method
	 * in this class may depend on.
	...
	private static class TranslateState {
		 ...
		 *                              Caller has to make sure this object is not used
		 *                              in other instances of TranslateState!
		 */
