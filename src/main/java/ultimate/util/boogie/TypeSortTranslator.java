...
public class TypeSortTranslator {
	...
	/**
	 * Constructor for ChcToBoogieTranslation. All sort-type pairs are passed as a
	 * HashRelation.
	 *
	 * @param sortToType
	 * @param script
	...
	/**
	 * Constructor is only used in a workaround for the backtranslation while
	 * dumping path programs.
	 */
	public TypeSortTranslator(final Script script, final Logger logger) {
		this(Collections.emptySet(), script, logger);
		{
	...
	/**
	 * Return the SMT sort for a boogie type. If the (type,sort) pair is not already
	 * stored in mtype2sort the
	 * corresponding sort is constructed and the pair (sort, type) is added to
	 * msort2type which is used for a
	 * backtranslation.
	 *
	 * @param astNode
	 ...
	 * mType2sort. Store the (sort, type) pair
	 * in msort2type.
	 *
	 * @param astNode
	 *                BoogieASTNode for which Sort is computed
	...
	protected Sort constructSort(final IBoogieType boogieType, final BoogieASTNode astNode) {
		try {
			...
			if ("Sort Array not declared".equals(e.getMessage())) {
				Boogie2SMT.reportUnsupportedSyntax(astNode, "Solver does not support arrays", logger);
				throw e;
			}
	...
	/**
	 * Construct the SMT sort for a Boogie type. Does not use any caching and,
	 * depending on your funAttributeCache, may
	 * create many sorts.
	 */
