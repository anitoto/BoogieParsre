...
import ultimate.models.IProgressMonitorService;
 ...
 * the toolchain from which it was
 * created.
 *
...
public final class MonitoredProcess implements IStorable {
	...
	/**
	 * Time in milliseconds to wait for the termination of a process after sending
	 * the exit command.
	 */
	...
	/**
	 * -2 because we wait until all 3 threads (stderr buffer, stdin buffer, actual
	 * process watcher) are ready.
	 */
	...
	private MonitoredProcess(final Process process, final String command, final String exitCommand, final Logger logger,
	...
	/**
	 * Start a new monitored process. The process will be terminated at the end of
	 * the toolchain. Note that you should
	 * not start an external process through some wrapper script, because Java will
	 * have trouble terminating this
	 * processes due to bug <a href=
	 * "http://bugs.java.com/bugdatabase/view_bug.do?bug_id=4770092">JDK-4770092</a>.
	 * If
	 * this occurs, Ultimate may deadlock because it cannot close input and output
	 * streams of the unresponsive process
	 * reliably.
	 *
	 * @param command
	 *                    A string array containing the command and its possible
	 *                    arguments that will be used to start a new
	 *                    process.
	 * @param workingDir
	 *                    A string describing the directory path of the working
	 *                    directory that should be used to start the
	 *                    process.
	 * @param exitCommand
	 *                    A string describing an "exit" command that should be sent
	 *                    to the standard input stream of the running
	 *                    process before trying to shut it down.
	 * @param logger
	 *                    An instance of Java Logger that should be used to report
	 *                    errors.
	 * @param storage
	 *                    An instance of {@link IToolchainStorage} that should be
	 *                    used to register this process for destruction
	...
	public static MonitoredProcess exec(final String[] command, final String workingDir, final String exitCommand,
			final Logger logger, final Storage storage) throws IOException {
		...
		if (workingDir == null) {
			...
			newMonitoredProcess = new MonitoredProcess(Runtime.getRuntime().exec(command, null, new File(workingDir)),
					oneLineCmd, exitCommand, logger, storage);
		}

		newMonitoredProcess.start(workingDir, oneLineCmd, storage);
	 ...
	 *
	 * @param command
	 *                    A command without arguments that will be used to start a
	 *                    new process.
	 * @param exitCommand
	 *                    A string describing an "exit" command that should be sent
	 *                    to the standard input stream of the running
	 ...
	 *                    errors.
	 * @param storage
	 *                    An instance of {@link Storage} that should be
	 *                    used to register this process for destruction
	 *                    after the current toolchain completes.
	 * @return A monitored process instance that represents the started process.
	 * @throws IOException
	 ...
	 *                     executable, this exception is thrown.
	 */
	public static MonitoredProcess exec(final String command, final String exitCommand, final Logger logger,
			final Storage storage) throws IOException {
		return exec(command.split(" "), null, exitCommand, logger, storage);
	}

	private void start(final String workingDir, final String oneLineCmd, final Storage storage) {
		...
		if (old != null) {
			logger.warning("Destroyed unexpected old storable " + key);
			old.destroy();
		}

		mMonitor = new Thread(new ProcessRunner(this), "MonitoredProcess " + mID + " " + oneLineCmd);
		logger.info(String.format("Starting monitored process %s with %s (exit command is %s, workingDir is %s)", mID,
	 ...
	 *         process is still running or not and if
	 *         not, what the return code is.
	 * @throws InterruptedException
	 *                              If any thread has interrupted the monitor
	 *                              thread. The interrupted status of the monitor
	 *                              thread is
	...
	/**
	 * Wait and block for some time for the normal termination of the process. If it
	 * did not occur, terminate the
	 * process abnormally.
	 *
	 * @param millis
	 *               The time in milliseconds for which the method waits for the
	 *               normal termination of the process. Must be
	 *               non-negative. A value of 0 means waiting forever.
	 * @return A {@link MonitoredProcessState} instance containing the return code
	 *         of the process or -1
	 */
	public MonitoredProcessState impatientWaitUntilTime(final long millis) {
		...
		logger.info(String.format("Waiting %s ms for monitored process %s with %s", millis, mID, mCommand));
	...
	/**
	 * Wait until the toolchain is cancelled for the termination of the process. If
	 * it did not occur, terminate the
	 * process abnormally.
	 *
	 * @param gracePeriod
	 *                    A time period in milliseconds that this method will wait
	 *                    after a toolchain cancellation request was
	 *                    received before terminating the process. Must be
	 *                    non-negative. 0 means no grace-period.
	 * @return A {@link MonitoredProcessState} instance containing the return code
	 *         of the process or -1
	 */
	public MonitoredProcessState impatientWaitUntilToolchainTimeout(final long gracePeriod) {
		...
		logger.info(String.format("Waiting until toolchain timeout for monitored process %s with %s", mID, mCommand));
		final IProgressMonitorService progressService = storage.getProgressMonitorService();
	 ...
	 * will be terminated forcefully. This
	 ...
	 * terminates, possibly after a grace
	 ...
	 *                    toolchain cancellation request was received
	 *                    before terminating the process. Must be non-negative. 0
	...
	/**
	 * Force the process to terminate regardless of its state. Will first try to use
	 * the defined exit command if there
	 * is any.
	 */
		...
		synchronized (this) {
			...
			if (mExitCommand != null) {
				...
				try {
					...
					logger.severe(getLogStringPrefix() + " Exception during sending of exit command " + mExitCommand
							+ ": " + e.getMessage());
				}
				try {
					logger.fine(getLogStringPrefix() + " About to join with the monitor thread... ");
					mMonitor.join(WAIT_FOR_EXIT_COMMAND_MILLIS);
					logger.fine(getLogStringPrefix() + " Successfully joined");

				} catch (final InterruptedException e) {
					// not necessary to do anything here
					logger.fine(getLogStringPrefix() + " Interrupted during join");
				}
				if (!isRunning()) {
					return;
				}
			}
			logger.warning(getLogStringPrefix() + " Forcibly destroying the process");
			final List<InputStream> tobeclosed = new ArrayList<>(5);
			try {
				// mProcess.destroyForcibly();
				// storage.removeStorable(getKey(mID, mCommand));
				...
				logger.severe(String.format(getLogStringPrefix() + " Something unexpected happened: %s%n%s", t,
			...
			logger.fine(getLogStringPrefix() + " Forcibly destroyed the process");
		}
	}
	...
	private void close(final Closeable pipe) {
		try {
			...
			logger.warning(getLogStringPrefix() + " An error occured during closing: " + e.getMessage());
		}
	}
	...
	/**
	 * @return the input stream connected to the error output of the subprocess. The
	 *         stream obtains data piped from the
	 *         error output of the process represented by this Process object. This
	 *         stream is already pumped to deal
	 *         with OS buffering.
	 */
	 ...
	 *         This stream is already pumped to deal
	 *         with OS buffering.
	 */
	...
	private void removeFrostorage() {
		final IStorable storable = storage.removeStorable(getKey(mID, mCommand));
		if (storable != null) {
			logger.fine(getLogStringPrefix() + " was removed from storage");
		}
	}
	...
	private final class ProcessRunner implements Runnable {
		...
		public void run() {
			...
			try {
				...
				mMonitoredProcess.logger.severe(
						getLogStringPrefix() + " Failed during stream data buffering. Terminating abnormally: "
								+ e.getMessage());
				killProcess();
			...
			try {
				...
				mMonitoredProcess.logger.fine(getLogStringPrefix() + " Finished waiting for process!");
				...
				if (!endOfPumps.tryAcquire(-INITIAL_SEMAPHORE_COUNT, WAIT_FOR_EXIT_COMMAND_MILLIS,
					...
					mMonitoredProcess.logger
							.warning(getLogStringPrefix() + " Abandoning pump threads because process wont die!");
				} else {
					mMonitoredProcess.logger.fine(getLogStringPrefix() + " Finished waiting for pump threads!");
					logUnreadPipeContent();
				}
			} catch (final InterruptedException e) {
				mMonitoredProcess.logger
						.severe(getLogStringPrefix() + " Interrupted. Terminating abnormally: " + e.getMessage());
			} finally {
				killProcess();
			}
		}
	...
	private final class PipePump implements Runnable {
		...
		public void run() {
			...
			try {
				...
				logger.warning(getLogStringPrefix() + " The stream was forcibly closed: " + mPumpName);
			} finally {
				try {
					mOutputStream.flush();
					mOutputStream.close();
				} catch (final IOException e) {
					logger.severe(getLogStringPrefix() + " During closing of the streams " + mPumpName
							+ ", an error occured");
				}
				mEndOfPumps.release();
			}
