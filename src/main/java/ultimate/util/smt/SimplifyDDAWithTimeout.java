...
public class SimplifyDDAWithTimeout extends SimplifyDDA {
	...
	public SimplifyDDAWithTimeout(final Script script, final Logger logger, final Storage storage) {
		this(script, true, logger, storage);
	}
	...
	public SimplifyDDAWithTimeout(final Script script, final boolean simplifyRepeatedly,
		...
		this(script, simplifyRepeatedly, logger, storage, null);
	}

	/**
	 * Constructor that allows simplification with respect to a given context. E.g.,
	 * (= x y) can be simplified to false
	 * if we assume that the term (and (= x 0) (= y 1)) is true. This can be used to
	 * apply the simplification only to
	 * subformulas of a formula. E.g., if we have the formula (and (= x 0) (= y 1)
	 * (= x y)) we can use the first two
	 * conjuncts as a (conjunctive) context and simplify only the third conjunct.
	 *
	 * @param context
	 *                Term that defines under which assumptions the simplification
	 *                is done.
	 */
	public SimplifyDDAWithTimeout(final Script script, final boolean simplifyRepeatedly,
			final Logger logger, final Storage storage, final Term context) {
	...
	protected Redundancy getRedundancy(final Term term) {
		if (!storage.getProgressMonitorService().continueProcessing()) {
			throw new RuntimeException(this.getClass() + " simplifying term of DAG size "
	...
	/**
	 * Copy&paste from original method where I removed the checktype modification
	...
	/**
	 * Copy&paste from original methods where I commented the assertions about the
	 * push-pop stack which are only
	 * supported by SMTInterpol. Furthermore, the assertion that checks the
	 * equivalence of the result was weakened. It
