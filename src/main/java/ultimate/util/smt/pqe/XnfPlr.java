...
public class XnfPlr extends XjunctPartialQuantifierElimination {
	...
	public XnfPlr(final ManagedScript script, final Logger logger) {
		super(script, logger);
	}
	...
	public Term[] tryToEliminate(final int quantifier, final Term[] dualJuncts, final Set<TermVariable> eliminatees) {
				...
				if (atom instanceof ApplicationTerm) {
					...
					if (aatom.getFunction().getName().equals("not")) {
						if (aatom.getParameters()[0].equals(var)) {
							logger.fine(String.format("eliminated quantifier via %s for %s", getAcronym(), var));
		...
		final SubstitutionWithLocalSimplification subst = new SubstitutionWithLocalSimplification(mMgdScript,
				substitutionMapping, logger);
