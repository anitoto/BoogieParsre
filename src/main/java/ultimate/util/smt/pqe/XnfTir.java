...
import ultimate.util.smt.linearterms.AffineRelation.TransformInequality;
...
public class XnfTir extends XjunctPartialQuantifierElimination {
	...
	public XnfTir(final ManagedScript script, final XnfConversionTechnique xnfConversionTechnique, final Logger logger, final Storage storage) {
		super(script, logger);
	...
	private List<Term> tryToEliminateConjuncts(final int quantifier, final Term[] inputAtoms,
		...
		for (final Term term : inputAtoms) {
			if (!Arrays.asList(term.getFreeVars()).contains(eliminatee)) {
				...
				final AffineRelation rel = AffineRelation.convert(mScript, term, transform, logger);
				if (rel == null) {
				...
				switch (bnr.getRelationSymbol()) {
					case DISTINCT:
						if (quantifier == QuantifiedFormula.EXISTS) {
							...
							assert occursInsideSelectTerm(term, eliminatee, logger) : "should have been removed by DER";
							// no chance to eliminate the variable
						}
						break;
					case EQ:
		...
		final BuildingInstructions bi = new BuildingInstructions(quantifier, eliminatee.getSort(),
				termsWithoutEliminatee, upperBounds, lowerBounds, antiDer, storage);
		...
		if (antiDer.isEmpty()) {
			...
			Term disjunction;
			if (quantifier == QuantifiedFormula.EXISTS) {
				disjunction = SmtUtils.toDnf(mMgdScript, tmp, mXnfConversionTechnique, logger, storage);
			} else if (quantifier == QuantifiedFormula.FORALL) {
				disjunction = SmtUtils.toCnf(mMgdScript, tmp, mXnfConversionTechnique, logger, storage);
			} else {
				throw new AssertionError("unknown quantifier");
			}
	 ...
	 */
	private static boolean occursInsideSelectTerm(final Term term, final TermVariable tv, final Logger logger) {
	...
	private Term buildInequality(final String symbol, final Term lhs, final Term rhs) {
		...
		final AffineRelation rel = AffineRelation.convert(mScript, term, logger);
		if (rel == null) {
