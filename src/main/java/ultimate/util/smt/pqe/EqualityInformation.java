...
import ultimate.util.smt.ContainsSubterm;
...
public class EqualityInformation {
	...
	public static EqualityInformation getEqinfo(final Script script, final Term givenTerm, final Term[] context,
			final Term forbiddenTerm, final int quantifier, final Logger logger) {
		...
		for (int i = 0; i < context.length; i++) {
			...
			final AffineRelation affRel = AffineRelation.convert(script, context[i], logger);
			if (affRel == null) {
				continue;
			}
	...
	public static Pair<Set<Term>, Set<Term>> getEqTerms(final Script script, final Term givenTerm, final Term[] context,
			final Term forbiddenTerm, final Logger logger) {
