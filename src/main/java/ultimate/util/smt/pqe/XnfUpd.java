...
import ultimate.util.smt.NonTheorySymbol;
import ultimate.util.smt.NonTheorySymbol.Constant;
...
public class XnfUpd extends XjunctPartialQuantifierElimination {
	...
	public XnfUpd(final ManagedScript script, final Logger logger) {
		super(script, logger);
	}
	...
	public Term[] tryToEliminate(final int quantifier, final Term[] dualJuncts, final Set<TermVariable> eliminatees) {
		...
		final ConnectionPartition connection = new ConnectionPartition(Arrays.asList(dualJuncts), logger);
		...
		for (final Set<NonTheorySymbol<?>> connectedSymbols : connection.getConnectedNonTheorySymbols()) {
			...
			if (containsNonTheoryConstant(connectedSymbols)) {
				...
				if (quantifier == QuantifiedFormula.EXISTS) {
					final Term simplified = isSuperfluousConjunction(mScript, connectedTerms, freeVarsOfConnectedTerms,
							eliminatees, logger);
					...
					final Term simplified = isSuperfluousDisjunction(mScript, connectedTerms, freeVarsOfConnectedTerms, eliminatees, logger);
		...
		if (removeableTerms.isEmpty()) {
			logger.fine("not eliminated quantifier via UPD for " + occuringVars);
			return dualJuncts;
		}
		eliminatees.removeAll(removeableTvs);
		logger.fine("eliminated quantifier via UPD for " + removeableTvs);
		final Term[] result;
