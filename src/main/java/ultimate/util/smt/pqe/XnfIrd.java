...
import ultimate.util.smt.linearterms.NotAffineException;
...
public class XnfIrd extends XjunctPartialQuantifierElimination {
	public XnfIrd(final ManagedScript script, final Logger logger) {
		super(script, logger);
	}
	...
	public Term[] tryToEliminate(final int quantifier, final Term[] dualJuncts, final Set<TermVariable> eliminatees) {
		...
		while (it.hasNext()) {
			...
			final Term[] withoutTv = irdSimple(mScript, quantifier, result, tv, logger);
			if (withoutTv != null) {
				logger.fine("eliminated quantifier via IRD for " + tv);
				result = withoutTv;
				it.remove();
			} else {
				logger.fine("not eliminated quantifier via IRD for " + tv);
			}
			// } else {
			// // ird is only applicable to variables of numeric sort
			// mLogger.debug(new DebugMessage("not eliminated quantifier via IRD for {0}",
			// tv));
			// }
		}
		return result;
	 ...
	 * param one of the following holds and
	 * the third case applies at most once, we return all params that do not contain
	 * tv. 1. param does not contain tv 2.
	 * param is an AffineRelation such that tv is a variable of the AffineRelation
	 * and the function symbol is "distinct"
	 * and quantifier is ∃ or the function symbol is "=" and the quantifier is ∀ 3.
	 * param is an inequality
	 *
	 * @param logger
	 */
	public static Term[] irdSimple(final Script script, final int quantifier, final Term[] oldParams,
			final TermVariable tv, final Logger logger) {
		...
		for (final Term oldParam : oldParams) {
			if (!Arrays.asList(oldParam.getFreeVars()).contains(tv)) {
				...
				final AffineRelation affineRelation = AffineRelation.convert(script, oldParam, logger);
	...
	/**
	 * @return true iff oldParam is a not equals relation (resp. quality for
	 *         universal quantifier) such that one side is
