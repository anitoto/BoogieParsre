...
import ultimate.util.smt.linearterms.BinaryRelation;
...
public class ArrayEqualityExplicator {
	...
	public ArrayEqualityExplicator(final ManagedScript mgdScript, final int quantifier, final TermVariable eliminatee,
			final Term inputTerm, final List<BinaryEqualityRelation> bers, final Logger logger) {
		this.logger = logger;
		...
		mResultTerm = new SubstitutionWithLocalSimplification(mgdScript, substitutionMapping, logger)
				.transform(inputTerm);
