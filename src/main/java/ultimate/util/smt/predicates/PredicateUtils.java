...
public class PredicateUtils {
	...
	/**
	 * Returns the DAG size of the predicate's formula. (DAG size means that similar
	 * sub-formulas are counted only
	 * once.)
	 */
	 ...
	 * (substituted by a constant that has the new
	 * name) according to the following scheme:
	 * <ul>
	 * <li>Each variable v that is contained in varsWithSpecialIdx is renamed to
	 * v_specialIdx
	 * <li>Each variable v that is not contained in varsWithSpecialIdx is renamed to
	 * v_defaultIdx
	 * <li>If oldVarIdx is Integer.MIN_VALUE, each oldVar v is renamed to v_OLD
	 * <li>For oldvars we check if the the corresponding non-old var is in the set
	 * of modifiableGlobals. If NO, the
	 * oldvar becomes the same constant as the corresponding non-old var. if YES the
	 * following applies. If oldVarIdx is
	 * not Integer.MIN_VALUE, each oldVar v is renamed to v_oldVarIdx. This means v
	 * can get the same name as a
	...
	/**
	 * Return formula of a transition where all variables are renamed (substituted
	 * by a constant that has the new name)
	 * according to the following scheme:
	 * <ul>
	 * <li>Each inVar v is renamed to v_idxInVar.
	 * <li>Each outVar v that does not occur as an inVar (no update/assignment) is
	 * renamed to v_idxOutVar.
	 * <li>Each variable v that occurs neither as inVar nor outVar is renamed to
	 * v_23.
	...
	public static LBool isInductiveHelper(final Script script, final IPredicate precond, final IPredicate postcond,
			...
			final Set<IProgramNonOldVar> modifiableGlobalsSucc, final Logger logger) {
		script.push(1);
		...
		{
			final Term postcondRenamed = rename(script, postcond, tf.getAssignedVars(), logger);
			conjuncts.add(SmtUtils.not(script, postcondRenamed, logger));
		}
	...
	/**
	 * Find all nonOldVars such that they are modifiable, their oldVar is in vars.
	 * Put the nonOldVar in
	...
	private static Term rename(final Script script, final IPredicate postcond, final Set<IProgramVar> assignedVars,
		...
		final Term result = (new Substitution(script, substitutionMapping, logger)).transform(postcond.getFormula());
