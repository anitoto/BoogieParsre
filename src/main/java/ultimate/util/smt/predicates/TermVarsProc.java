...
public class TermVarsProc {
	 ...
	 * Compute the BoogieVars of the free
	...
	public static TermVarsProc computeTermVarsProc(final Term term, final Script script,
			final IIcfgSymbolTable symbolTable, final Logger logger) {
		return computeTermVarsProc(term, script, symbolTable::getProgramVar, logger);
	}
	 ...
	 */
	public static TermVarsProc computeTermVarsProc(final Term term, final Script script,
			final Function<TermVariable, IProgramVar> funTermVar2ProgVar, final Logger logger) {
		...
		final Term closedTerm = PredicateUtils.computeClosedFormula(term, vars, script, logger);
	 ...
	 * Given a term in which every free variable is the TermVariable of a BoogieVar.
	 * Compute the BoogieVars of the free
	 * variables and the procedures of these BoogieVariables. If
	 * replaceNonModifiableOldVars is true, modifiableGlobals
	 * must be non-null and we check we replace the oldVars of all non-modifiable
	 * Globals by their corresponding
	 * non-oldVars.
	 *
	 * 2015-05-27 Matthias: At the moment, I don't know if we need this method.
	 * Don't use it unless you know what you
	 * do.
	 */
	@Deprecated
	private static TermVarsProc computeTermVarsProc(Term term, final Boogie2SMT boogie2smt,
			final boolean replaceNonModifiableOldVars, final Set<IProgramVar> modifiableGlobals, final Logger logger) {
		final HashSet<IProgramVar> vars = new HashSet<>();
		...
		if (!oldVarsThatHaveToBeReplaced.isEmpty()) {
			...
			term = new Substitution(boogie2smt.getScript(), substitutionMapping, logger).transform(term);
		}
		final Term closedTerm = PredicateUtils.computeClosedFormula(term, vars, boogie2smt.getScript(), logger);
		return new TermVarsProc(term, vars, procs.toArray(new String[procs.size()]), closedTerm);
	}
