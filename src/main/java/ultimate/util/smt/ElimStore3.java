...
import ultimate.util.smt.arrays.ArrayUpdate;
import ultimate.util.smt.arrays.ArrayUpdate.ArrayUpdateException;
import ultimate.util.smt.arrays.ArrayUpdate.ArrayUpdateExtractor;
...
public class ElimStore3 {
	...
	public ElimStore3(final Script script, final ManagedScript mgdScript,
	...
	private static MultiDimensionalStore getArrayStore(final Term array, final Term term, final Logger logger) {
		final List<MultiDimensionalStore> all = MultiDimensionalStore.extractArrayStoresDeep(term, logger);
	...
	private ArrayUpdate getArrayUpdate(final Term array, final Term[] terms) {
		final ArrayUpdateExtractor aue = new ArrayUpdateExtractor(logger, mQuantifier == QuantifiedFormula.FORALL, true,
				terms);
	...
	public Term elim(final int quantifier, final TermVariable inputeliminatee, final Term inputterm,
		...
		while (true) {
			...
			MultiDimensionalStore store = getArrayStore(eliminatee, term, logger);
			...
			if ((store != null || update != null) && (writeInto == null && writtenFrom == null)) {
				final TermVariable auxArray = mMgdScript.constructFreshTermVariable(s_FreshVariableString,
						eliminatee.getSort());
				if (store == null) {
				...
				final Substitution subst = new SubstitutionWithLocalSimplification(mMgdScript, auxMap, logger);
				...
				if (quantifier == QuantifiedFormula.EXISTS) {
					...
					assert quantifier == QuantifiedFormula.FORALL;
					auxTerm = SmtUtils.or(mScript, auxTerm, SmtUtils.not(mScript, auxVarDef, logger));
				}
		...
		if (writeInto != null) {
			...
			final Substitution substStoreTerm = new SubstitutionWithLocalSimplification(mMgdScript, mapping, logger);
			intermediateResult = substStoreTerm.transform(intermediateResult);
		}

		// Indices and corresponding values of a_elim
		final IndicesAndValues iav = new IndicesAndValues(logger, mMgdScript, quantifier, eliminatee, conjuncts);
		final Substitution subst = new SubstitutionWithLocalSimplification(mMgdScript, iav.getMapping(), logger);
		...
		if (write) {
			...
			/**
			 * Special treatment for the case that the eliminatee to which we write occurs
			 * also in a term somewhere else
			 * (which is not a select term). Maybe we can avoid this if we eliminate
			...
			if (writtenFrom != null) {
				...
				writtenFromSubst = new SubstitutionWithLocalSimplification(mMgdScript, mapping, logger);
			} else {
				writtenFromSubst = null;
			}

			if (quantifier == QuantifiedFormula.EXISTS) {
				final Term additionalConjuncts = SmtUtils.and(script,
						additionalConjuncs.toArray(new Term[additionalConjuncs.size()]));
				intermediateResult = SmtUtils.and(script, intermediateResult, additionalConjuncts);
			} else {
				assert quantifier == QuantifiedFormula.FORALL;
				final Term additionalConjuncts = SmtUtils.or(script,
						SmtUtils.negateElementwise(mScript, additionalConjuncs, logger)
		...
		final ArrayList<Term> indexValueConstraintsFromEliminatee;
		{
			...
			final Pair<List<ArrayIndex>, List<Term>> indicesAndValues = buildIndicesAndValues(mgdScript, iav, logger);
			...
			if (writtenFrom != null) {
				...
				final ArrayIndex idx_writeRenamed = new ArrayIndex(
						SmtUtils.substitutionElementwise(writtenFrom.getIndex(), subst));
		...
		final Term result1 = QuantifierUtils.applyDualFiniteConnective(mScript, mQuantifier,
		...
		Term result = QuantifierUtils.applyDualFiniteConnective(mScript, mQuantifier,
				Arrays.asList(new Term[] { result1, intermediateResult }));
		// if (quantifier == QuantifiedFormula.EXISTS) {
		// final Term newConjunctsFromSelect = SmtUtils.and(mScript,
		// indexValueConstraintsFromEliminatee.toArray(new
		// Term[indexValueConstraintsFromEliminatee.size()]));
		// result = SmtUtils.and(script, intermediateResult, newConjunctsFromSelect);
		// } else {
		// assert quantifier == QuantifiedFormula.FORALL;
		// final Term newConjunctsFromSelect =
		// SmtUtils.or(mScript, SmtUtils.negateElementwise(mScript,
		// indexValueConstraintsFromEliminatee)
		...
		result = SmtUtils.simplify(mMgdScript, result, mSimplificationTechnique, logger, storage);
	...
	public static Pair<List<ArrayIndex>, List<Term>> buildIndicesAndValues(final ManagedScript mgdScript,
			final IndicesAndValues iav, final Logger logger) {
		final List<ArrayIndex> indices1 = new ArrayList<>();
		final List<Term> values1 = new ArrayList<>();
		final Substitution subs = new SubstitutionWithLocalSimplification(mgdScript, iav.getMapping(), logger);
		for (int i = 0; i < iav.getIndices().length; i++) {
			final ArrayIndex translatedIndex = new ArrayIndex(
					SmtUtils.substitutionElementwise(iav.getIndices()[i], subs));
	...
	public static ArrayList<Term> constructIndexValueConstraints(final Script script, final int quantifier,
			final List<ArrayIndex> indices, final List<Term> values, final Logger logger) {
		final ArrayList<Term> indexValueConstraintsFromEliminatee;
		...
		for (int i = 0; i < indices.size(); i++) {
			for (int j = i; j < indices.size(); j++) {
				Term newConjunct = SmtUtils.indexEqualityImpliesValueEquality(script, indices.get(i),
						indices.get(j), values.get(i), values.get(j), logger);
				if (quantifier == QuantifiedFormula.FORALL) {
					newConjunct = SmtUtils.not(script, newConjunct, logger);
				}
	...
	/**
	 * let idx_write be the index to which the store writes let k_1,...,k_n be
	 * indices of the eliminated array let
	 * v_1,...,v_n be terms that are equivalent to the corresponding values of the
	 * eliminated array (i.e. v_i is
	 * equivalent to a_elim[k_i] add for each i the conjunct (idx_write != k_i) ==>
	 * (v_i == a_heir[i])
	 *
	 * Says that for each index that is different from the write index the
	 * arrayCells of a_heir have the same values
	...
	// public static Term indexValueConnections(ArrayIndex ourIndex, Term ourValue,
	// ArrayIndex[] othersIndices,
	...
	// Term indexEquality = SmtUtils.and(script, buildPairwiseEquality(ourIndex,
	// othersIndices[i], null, script));
	// Term valueEquality = SmtUtils.binaryEquality(script, ourValue,
	// othersValues[i]);
	// Term conjunct = SmtUtils.or(script, SmtUtils.not(script, indexEquality),
	// valueEquality);
	// if (quantifier == QuantifiedFormula.EXISTS) {
	...
	// Term result;
	// if (quantifier == QuantifiedFormula.EXISTS) {
	// result = SmtUtils.and(script, additionalConjuncs.toArray(new
	// Term[additionalConjuncs.size()]));
	// } else {
	// assert quantifier == QuantifiedFormula.FORALL;
	// result = SmtUtils.or(script, additionalConjuncs.toArray(new
	// Term[additionalConjuncs.size()]));
	// }
	//
	//
	...
	/**
	 * Given an array a, find all multi-dimensional selects on this array. For each
	 * of them, try to obtain an
	 * representation of that value in which a does not occur. If not such
	 * representation exists, add a new auxiliary
	...
	public static class IndicesAndValues {
		...
		public IndicesAndValues(final Logger logger, final ManagedScript mgdScript, final int quantifier,
				final TermVariable array, final Term... conjuncts) {
			mMgdScript = mgdScript;
			mQuantifier = quantifier;
			...
			for (int i = 0; i < arrayReads.length; i++) {
				...
				final EqualityInformation eqInfo = EqualityInformation.getEqinfo(mMgdScript.getScript(),
						arrayReads[i].getSelectTerm(), conjuncts, array, mQuantifier, logger);
				if (eqInfo == null) {
					final Term select = arrayReads[i].getSelectTerm();
					final TermVariable auxVar = mMgdScript.constructFreshTermVariable(s_FreshVariableString,
							select.getSort());
	...
	/**
	 * Class that constructs the term (fstIndex == sndIndex) ==> (fstValue ==
	 * sndValue) if selectConnection is true and
	 * the term (fstIndex != sndIndex) ==> (fstValue == sndValue) if
	 * selectConnection is false.
	 *
	 */
	private class IndexValueConnection {
		...
		public IndexValueConnection(final ArrayIndex fstIndex, final ArrayIndex sndIndex, final Term fstValue,
			...
			mIndexEquality = SmtUtils.and(mScript, SmtUtils.pairwiseEquality(mScript, fstIndex, sndIndex, logger));
			mValueEquality = SmtUtils.binaryEquality(mScript, fstValue, sndValue, logger);
		}
		...
		public Term getTerm() {
			...
			if (mSelectConnection) {
				indexTerm = SmtUtils.not(mScript, indexTerm, logger);
			}
	...
	private Term eliminateSelfUpdates(final Script script, final int quantifier, final Term term) {
		...
		// ArrayUpdateExtractor aue = new ArrayUpdateExtractor(quantifier ==
		// QuantifiedFormula.FORALL, false,
		// conjuncts);
		...
		for (final Term conjunct : conjuncts) {
			ArrayUpdate au;
			try {
				au = new ArrayUpdate(conjunct, false, false, logger);
			} catch (final ArrayUpdateException aue1) {
				try {
					au = new ArrayUpdate(conjunct, true, false, logger);
