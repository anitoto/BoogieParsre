...
public class AffineTerm extends Term {
	 ...
	 */
	public AffineTerm(final Sort s, final Rational r, final Logger logger) {
		super(0);
		this.logger = logger;
		mSort = s;
		mConstant = r;
	 ...
	 * the {@link Term} t.
	 */
	public AffineTerm(final Term t, final Logger logger) {
		super(0);
		this.logger = logger;
		mSort = t.getSort();
	 ...
	 * corresponding coefficients are given by the
	 * array coefficients, and whose constant term is given by the Rational
	 * constant.
	 */
	 ...
	 */
	public AffineTerm(final Logger logger, final AffineTerm... affineTerms) {
		super(0);
		this.logger = logger;
	 ...
	 */
	public AffineTerm(final AffineTerm affineTerm, final Rational multiplier, final Logger logger) {
		super(0);
		this.logger = logger;
	 ...
	 * process, e.g., if original term was not
	 * linear.
	 */
	public AffineTerm(final Logger logger) {
		super(0);
		this.logger = logger;
	 ...
	 * translation process, e.g., if original term
	 * was not linear.
	 */
	...
	public static AffineTerm applyModuloToAllCoefficients(final Script script, final AffineTerm affineTerm,
			final BigInteger divident, final Logger logger) {
		...
		for (final Entry<Term, Rational> entry : map.entrySet()) {
			...
			coefficients[offset] = SmtUtils
					.toRational(BoogieUtils.euclideanMod(SmtUtils.toInt(entry.getValue()), divident), logger);
			offset++;
		}
		final Rational constant = SmtUtils
				.toRational(BoogieUtils.euclideanMod(SmtUtils.toInt(affineTerm.getConstant()), divident), logger);
		return new AffineTerm(affineTerm.getSort(), terms, coefficients, constant, logger);
	}
