...
import ultimate.logic.INonSolverScript;
...
public class AffineRelation {
	 ...
	 * equalities (i.e., by keeping the
	 * natural relation of the term).
	 *
	 * @param term
	 * @throws NotAffineException
	 */
	public AffineRelation(final Script script, final Term term, final Logger logger) throws NotAffineException {
		this(script, term, TransformInequality.NO_TRANFORMATION, logger);
	}
	 ...
	 *
	 * @param term
	 *                            Term to which the resulting AffineRelation is
	 *                            equivalent.
	 * @param transformInequality
	 *                            transform strict inequalities to non-strict
	...
	public AffineRelation(final Script script, final Term term, final TransformInequality transformInequality,
			final Logger logger) throws NotAffineException {
		this.logger = logger;
		mOriginalTerm = term;
		...
		final AffineTerm affineLhs = (AffineTerm) new AffineTermTransformer(script, logger).transform(lhs);
		final AffineTerm affineRhs = (AffineTerm) new AffineTermTransformer(script, logger).transform(rhs);
		if (affineLhs.isErrorTerm() || affineRhs.isErrorTerm()) {
			throw new NotAffineException("Relation is not affine");
		}
		final AffineTerm difference = new AffineTerm(logger, affineLhs,
				new AffineTerm(affineRhs, Rational.MONE, logger));
			...
			if (transformInequality == TransformInequality.STRICT2NONSTRICT) {
				switch (bnr.getRelationSymbol()) {
					...
					case LESS:
						...
						mAffineTerm = new AffineTerm(logger, difference,
								new AffineTerm(difference.getSort(), Rational.ONE, logger));
						break;
					case GREATER:
						// decrement affine term by one
						mRelationSymbol = RelationSymbol.GEQ;
						mAffineTerm = new AffineTerm(logger, difference,
								new AffineTerm(difference.getSort(), Rational.MONE, logger));
	 ...
	 * occurs only positive and the greater-than
	 * relation symbols are replaced by less-than relation symbols. If the term is
	 * equivalent to <i>true</i> (resp.
		...
		if (mTrivialityStatus == TrivialityStatus.EQUIVALENT_TO_TRUE) {
			...
			for (final Entry<Term, Rational> entry : mAffineTerm.getVariable2Coefficient().entrySet()) {
				if (entry.getValue().isNegative()) {
					rhsSummands.add(product(script, entry.getValue().abs(), entry.getKey(), logger));
				} else {
					lhsSummands.add(product(script, entry.getValue(), entry.getKey(), logger));
				}
			}
			...
			assert script instanceof INonSolverScript
					|| isEquivalent(script, mOriginalTerm, result) != LBool.SAT
					: "transformation to positive normal form "
							+ "unsound";
			return result;
		}
	}
	 ...
	 * (note that in our AffineTerms the
	 * variables may be SMT terms like e.g., a select term) is on the left hand side
	 * with coeffcient one. Throw a
	 * NotAffineException if no such representation is possible (e.g, if the
	 * variable does not occur in the term, or the
		...
		for (final Entry<Term, Rational> entry : mAffineTerm.getVariable2Coefficient().entrySet()) {
			if (var == entry.getKey()) {
				...
				if (newCoeff.isIntegral() || SmtSortUtils.isRealSort(mAffineTerm.getSort())) {
					...
					rhsSummands.add(product(script, negated, entry.getKey(), logger));
				} else {
	...
	private static Term product(final Script script, final Rational rational, final Term term, final Logger logger) {
		if (rational.equals(Rational.ONE)) {
			return term;
		} else if (rational.equals(Rational.MONE)) {
			return SmtUtils.neg(script, term, logger);
		} else {
	...
	public static AffineRelation convert(final Script script, final Term term, final Logger logger) {
		return convert(script, term, TransformInequality.NO_TRANFORMATION, logger);
	}

	public static AffineRelation convert(final Script script, final Term term,
			final TransformInequality transformInequality, final Logger logger) {
		final BinaryNumericRelation bnr = BinaryNumericRelation.convert(term);
		if (bnr == null) {
			return null;
		}
			...
			if (transformInequality == TransformInequality.STRICT2NONSTRICT) {
				switch (bnr.getRelationSymbol()) {
					...
					case LESS:
						...
						affineTerm = new AffineTerm(logger, difference,
								new AffineTerm(difference.getSort(), Rational.ONE, logger));
						break;
					case GREATER:
						// decrement affine term by one
		...
		return new AffineRelation(script, affineTerm, relationSymbol, logger);
	}
