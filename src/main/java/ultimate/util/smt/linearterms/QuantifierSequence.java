...
public class QuantifierSequence {
	...
	public QuantifierSequence(final Script script, final Term input, final Logger logger) {
		this.logger = logger;
		mScript = script;
		...
		for (int i = quantifierSequence.size() - 1; i >= 0; i--) {
			...
			result = script.quantifier(quantifiedVars.getQuantifier(),
					quantifiedVars.getVariables().toArray(
							new TermVariable[quantifiedVars.getVariables().size()]),
					result);
		}
		return result;
	}
	...
	public void replace(final Set<TermVariable> forbiddenVariables,
		...
		mInnerTerm = (new Substitution(mScript, substitutionMapping, logger)).transform(mInnerTerm);
	}
	...
	public static Term mergeQuantifierSequences(final ManagedScript mgdScript, final String functionSymbolName,
		...
		final List<QuantifiedVariables> resultQuantifierBlocks = new ArrayList<QuantifiedVariables>(
				largestSequence.getNumberOfQuantifierBlocks());
	...
	private static void integrateQuantifierBlocks(final List<QuantifiedVariables> resultQuantifierBlocks,
		...
		final int offset;
		{
			final int lastQuantifierResult = resultQuantifierBlocks.get(resultQuantifierBlocks.size() - 1)
					.getQuantifier();
