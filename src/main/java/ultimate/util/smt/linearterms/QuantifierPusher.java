...
import ultimate.util.smt.SubTermFinder;
...
import ultimate.util.smt.pqe.XnfPlr;
 ...
 * possible and quantifiers are eliminated via
...
public class QuantifierPusher extends TermTransformer {
	...
	public enum PqeTechniques {
		...
		ONLY_DER,
		/**
		 * Apply all our partial quantifier elimination techniques that can be applied
		 * locally to subterms without
	...
	/**
	 * Try to apply distributivity rules to get connectives over which we can push
	 * quantifiers. E.g. if we have a
	 * formula or the form (A && (B || C)) we cannot directly push an existential
	 * quantifier. We can apply
	 * distributivity, obtain (A && B) || (A && C) and can now push the existential
	 * quantifier one step further.
	 */
	private final boolean mApplyDistributivity;

	public QuantifierPusher(final ManagedScript script, final boolean applyDistributivity,
			final PqeTechniques quantifierEliminationTechniques, final Logger logger, final Storage storage) {
		this.logger = logger;
	...
	private Term tryToPush(QuantifiedFormula quantifiedFormula) throws AssertionError {
		SubformulaClassification classification = classify(quantifiedFormula.getQuantifier(),
	...
	private Term tryToPushOverDualFiniteConnective(final QuantifiedFormula quantifiedFormula) {
		...
		final ParameterPartition pp = new ParameterPartition(mScript, quantifiedFormula, logger);
					...
					for (int i = 0; i < dualFiniteParams.length; i++) {
						if (isCorrespondingFinite(dualFiniteParams[i], quantifier)) {
							final Term correspondingFinite = applyDistributivityAndPushOneStep(quantifier, eliminatees,
									dualFiniteParams, i);
							final Term pushed = new QuantifierPusher(mMgdScript, mApplyDistributivity, mPqeTechniques,
									logger, storage).transform(correspondingFinite);
	...
	private Term applyDistributivityAndPushOneStep(final int quantifier, final Set<TermVariable> eliminatees,
		...
		for (final Term cfp : correspondingFiniteParams) {
			...
			resultOuterParams[offset] = QuantifierUtils.applyDualFiniteConnective(mScript, quantifier,
					resultInnerParams);
			resultOuterParams[offset] = SmtUtils.quantifier(mScript, quantifier, eliminatees,
	...
	private Term applyEliminationTechniques(final int quantifier, final Set<TermVariable> eliminatees,
		...
		switch (mPqeTechniques) {
			case ALL_LOCAL:
				elimtechniques.add(new XnfPlr(mMgdScript, logger));
				elimtechniques.add(new XnfDer(mMgdScript, logger, storage));
				elimtechniques.add(new XnfIrd(mMgdScript, logger));
				elimtechniques
						.add(new XnfTir(mMgdScript, XnfConversionTechnique.BOTTOM_UP_WITH_LOCAL_SIMPLIFICATION, logger,
								storage));
				elimtechniques.add(new XnfUpd(mMgdScript, logger));
				break;
			case NO_UPD:
				elimtechniques.add(new XnfPlr(mMgdScript, logger));
				elimtechniques.add(new XnfDer(mMgdScript, logger, storage));
	...
	private SubformulaClassification classify(final int quantifier, final Term subformula) {
		if (subformula instanceof QuantifiedFormula) {
			...
			final String dualFiniteConnective = SmtUtils
					.getCorrespondingFiniteConnective(SmtUtils.getOtherQuantifier(quantifier));
	...
	private Term processDualQuantifier(final QuantifiedFormula quantifiedFormula) {
		...
		final Term quantifiedSubFormulaPushed = (new QuantifierPusher(mMgdScript, mApplyDistributivity, mPqeTechniques,
				logger, storage))
