...
import ultimate.util.smt.BitvectorUtils;
...
public class AffineTermTransformer extends TermTransformer {
	...
	public AffineTermTransformer(final Script script, final Logger logger) {
		this.logger = logger;
		mScript = script;
	}
	...
	protected void convert(final Term term) {
		...
		final Rational valueOfLiteral = tryToConvertToLiteral(mScript, term, logger);
		if (valueOfLiteral != null) {
			final AffineTerm result = new AffineTerm(term.getSort(), valueOfLiteral, logger);
			setResult(result);
			return;
		}
		...
		if (mIsAffineVariable.test(term)) {
			final AffineTerm result = new AffineTerm(term, logger);
			setResult(result);
			return;
		}
	...
	private void inputIsNotAffine() {
		setResult(new AffineTerm(logger));
	}
	 ...
	 */
	private static Rational tryToConvertToLiteral(final Script script, final Term term, final Logger logger) {
		...
		if (SmtSortUtils.isBitvecSort(term.getSort())) {
			...
			if (term instanceof ConstantTerm) {
				result = SmtUtils.convertConstantTermToRational((ConstantTerm) term, logger);
			} else {
				result = null;
			}
		} else {
	...
	public void convertApplicationTerm(final ApplicationTerm appTerm, final Term[] newArgs) {
		...
		if (funName.equals("*") || funName.equals("bvmul")) {
			...
			final AffineTerm result = tryToMultiply(sort, affineArgs, logger);
			if (result == null) {
				inputIsNotAffine();
				return;
			}
			...
			final AffineTerm result = add(affineArgs, logger);
			setResult(result);
			return;
			...
			if (affineArgs.length == 1) {
				// unary minus
				result = negate(affineArgs[0], logger);
			} else {
				result = subtract(affineArgs, logger);
			}
			setResult(result);
			return;
		} else if (funName.equals("/")) {
			final Sort sort = appTerm.getSort();
			final AffineTerm result = divide(sort, affineArgs, logger);
			if (result == null) {
				inputIsNotAffine();
				return;
			}
	 ...
	 */
	private static AffineTerm add(final AffineTerm[] affineArgs, final Logger logger) {
		final AffineTerm result = new AffineTerm(logger, affineArgs);
		return result;
	}
	 ...
	 */
	private static AffineTerm negate(final AffineTerm affineTerm, final Logger logger) {
		return new AffineTerm(affineTerm, Rational.MONE, logger);
	}
	 ...
	 * <code>t1-(t2+...+tn)</code>
	 */
	private static AffineTerm subtract(final AffineTerm[] input, final Logger logger) {
		...
		for (int i = 1; i < argumentsForSum.length; i++) {
			argumentsForSum[i] = new AffineTerm(input[i], Rational.MONE, logger);
		}
		// construct the sum
		return add(argumentsForSum, logger);
	}
	 ...
	 */
	private static AffineTerm tryToMultiply(final Sort sort, final AffineTerm[] affineTerms, final Logger logger) {
		AffineTerm result;
		...
		if (nonLiteralArgument == null) {
			result = new AffineTerm(sort, multiplier, logger);
		} else {
			result = new AffineTerm(nonLiteralArgument, multiplier, logger);
		}
		return result;
	}
	 ...
	 */
	private static AffineTerm divide(final Sort sort, final AffineTerm[] affineArgs, final Logger logger) {
		...
		if (affineTerm == null) {
			...
			result = new AffineTerm(affineTerm, multiplier, logger);
		}
		return result;
	}
	 ...
	 *
	 */
	private static AffineTerm convertConstantNumericTerm(final ConstantTerm constTerm, final Logger logger) {
		final Rational rational = SmtUtils.convertConstantTermToRational(constTerm, logger);
		final AffineTerm result = new AffineTerm(constTerm.getSort(), rational, logger);
		return result;
	}
	 ...
	 */
	private static AffineTerm convertToReal(final ApplicationTerm term, final Logger logger) {
		...
		if (param instanceof ConstantTerm) {
			...
			final AffineTerm intTerm = convertConstantNumericTerm(constant, logger);
			final AffineTerm realTerm = new AffineTerm(term.getSort(), intTerm.getConstant(), logger);
			result = realTerm;
		} else {
			result = new AffineTerm(term, logger);
		}
		return result;
	}
