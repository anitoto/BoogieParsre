...
public class SubstitutionWithLocalSimplification extends Substitution {
	...
	public SubstitutionWithLocalSimplification(final ManagedScript mgdScript,
			final Map<Term, Term> substitutionMapping, final Logger logger) {
		super(mgdScript, substitutionMapping, logger);
	}
	...
	public void convertApplicationTerm(final ApplicationTerm appTerm, final Term[] newArgs) {
		...
		if (oldArgs == newArgs) {
			...
			result = SmtUtils.termWithLocalSimplification(logger, mMgdScript.getScript(), appTerm.getFunction(),
					newArgs);
		}
		setResult(result);
	}
