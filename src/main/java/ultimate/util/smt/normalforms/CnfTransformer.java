...
public class CnfTransformer extends XnfTransformer {
	...
	public CnfTransformer(final ManagedScript script, final Logger logger, final Storage storage) {
		this(script, false, logger, storage);
	}

	public CnfTransformer(final ManagedScript script, final boolean omitSoundnessCheck, final Logger logger, final Storage storage) {
		super(script, omitSoundnessCheck, logger, storage);
	}

	@Override
	protected NnfTransformerHelper getNnfTransformerHelper(final Logger logger) {
		return new CnfTransformerHelper(logger);
	}

	protected class CnfTransformerHelper extends XnfTransformerHelper {
		protected CnfTransformerHelper(final Logger logger) {
			super(logger);
		}
