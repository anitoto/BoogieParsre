...
import ultimate.util.smt.QuantifierUtils;
...
public class NnfTransformer {
	...
	public enum QuantifierHandling {
		 ...
		 * normal form. 2018-06-01
		 * Matthias: better use {@link PrenexNormalForm} instead, PULL may produce
		 * formulas with more quantifier
		 * alternations.
		 */
		PULL
	}
	...
	/**
	 * Sometimes we need to omit the soundness check which does a checksat on
	 * mManagedScript. For example, this is the
	 * case when mManagedScript.getScript is HornClauseParserScript (in which case
	 * the soundness check would lead to
	 * nontermination)
	 */
	private final boolean mOmitSoundnessCheck;

	public NnfTransformer(final ManagedScript mgdScript, final QuantifierHandling quantifierHandling,
			final Logger logger) {
		this(mgdScript, quantifierHandling, false, a -> false, logger);
	}

	public NnfTransformer(final ManagedScript mgdScript, final QuantifierHandling quantifierHandling,
			final boolean omitSoundnessCheck, final Logger logger) {
		this(mgdScript, quantifierHandling, omitSoundnessCheck, a -> false, logger);
	}
	...
	public NnfTransformer(final ManagedScript mgdScript, final QuantifierHandling quantifierHandling,
		...
		mNnfTransformerHelper = getNnfTransformerHelper(logger);
		mOmitSoundnessCheck = omitSoundnessCheck;
	}

	protected NnfTransformerHelper getNnfTransformerHelper(final Logger logger) {
		return new NnfTransformerHelper(logger);
	}
	...
	public Term transform(final Term term) {
		...
		if (mQuantifierHandling == QuantifierHandling.PULL) {
			for (int i = 0; i < mQuantifiedVariables.size(); i++) {
				final TermVariable[] variables = mQuantifiedVariables.get(i)
						.toArray(new TermVariable[mQuantifiedVariables.get(i).size()]);
		...
		// mScript.term("distinct", term, result)) != LBool.SAT : "Nnf transformation
		// unsound";
		return result;
	}
	...
	protected class NnfTransformerHelper extends TermTransformer {
		...
		protected NnfTransformerHelper(final Logger logger) {
			this.logger = logger;
		}
		...
		protected void convert(final Term term) {
			...
			if (term instanceof ApplicationTerm) {
				...
				if (functionName.equals("and")) {
					...
					convert(SmtUtils.or(mScript, negateAllButLast(mScript, params, logger)));
					return;
					...
					if (params.length > 2) {
						...
						// the argument of this call might have been simplified
						// to a term whose function symbol is neither "and" nor "or"
						convert(SmtUtils.binaryBooleanEquality(mScript, params[0], params[1], logger));
					}
				} else if (isXor(appTerm, functionName, logger)) {
					final Term[] params = appTerm.getParameters();
					if (params.length > 2) {
						final Term binarized = SmtUtils.binarize(mScript, appTerm);
						// we deliberately call convert() instead of super.convert()
						...
						// the argument of this call might have been simplified
						// to a term whose function symbol is neither "and" nor "or"
						convert(SmtUtils.binaryBooleanNotEquals(mScript, params[0], params[1], logger));
					}
					...
					final Term result = convertIte(mScript, condTerm, ifTerm, elseTerm, logger);
					// we deliberately call convert() instead of super.convert()
					// the argument of this call might have been simplified
					// to a term whose function symbol is neither "and" nor "or"
					convert(result);
				...
				switch (mQuantifierHandling) {
					...
					case PULL: {
						...
						for (final TermVariable oldTv : qf.getVariables()) {
							final TermVariable freshTv = mMgdScript.constructFreshTermVariable(s_FreshVariableString,
									oldTv.getSort());
							substitutionMapping.put(oldTv, freshTv);
							variables.add(freshTv);
						}
						final Term newBody = new Substitution(mScript, substitutionMapping, logger)
								.transform(qf.getSubformula());
						// we deliberately call convert() instead of super.convert()
						// the argument of this call might have been simplified
						// to a term whose function symbol is neither "and" nor "or"
						convert(newBody);
				...
				logger.warning("thrown away annotations " + Arrays.toString(((AnnotatedTerm) term).getAnnotations()));
		...
		private void convertNot(final Term notParam, final Term notTerm) {
			...
			final Term pushed = pushNot1StepInside(mScript, notParam, mQuantifierHandling, logger);
			if (pushed == null) {
				setResult(notTerm);
			} else {
				// we deliberately call convert() instead of super.convert()
		...
		public void convertApplicationTerm(final ApplicationTerm appTerm, final Term[] newArgs) {
			final Term simplified = SmtUtils.termWithLocalSimplification(logger, mScript, appTerm.getFunction(),
					newArgs);
	...
	private static Term[] negateTerms(final Script script, final Term[] terms, final Logger logger) {
		final Term[] newTerms = new Term[terms.length];
		for (int i = 0; i < terms.length; i++) {
			newTerms[i] = SmtUtils.not(script, terms[i], logger);
		}
		return newTerms;
	}

	private static Term[] negateLast(final Script script, final Term[] terms, final Logger logger) {
		final Term[] newTerms = new Term[terms.length];
		System.arraycopy(terms, 0, newTerms, 0, terms.length - 1);
		newTerms[terms.length - 1] = SmtUtils.not(script, terms[terms.length - 1], logger);
		return newTerms;
	}

	private static Term[] negateAllButLast(final Script script, final Term[] terms, final Logger logger) {
		final Term[] newTerms = new Term[terms.length];
	...
	public static Term convertIte(final Script script, final Term condTerm, final Term ifTerm, final Term elseTerm,
			final Logger logger) {
		final Term condImpliesIf = SmtUtils.or(script, SmtUtils.not(script, condTerm, logger), ifTerm);
	 ...
	 * </ul>
	 */
	public static boolean isXor(final ApplicationTerm appTerm, final String functionName, final Logger logger) {
	...
	public static Term pushNot1StepInside(final Script script, final Term notParam,
			final QuantifierHandling quantifierHandling, final Logger logger) {
		if (notParam instanceof ApplicationTerm) {
			final ApplicationTerm appTerm = (ApplicationTerm) notParam;
			final String functionName = appTerm.getFunction().getName();
			final Term[] params = appTerm.getParameters();
			if (functionName.equals("and")) {
				return SmtUtils.or(script, negateTerms(script, params, logger));
			} else if (functionName.equals("or")) {
				return SmtUtils.and(script, negateTerms(script, params, logger));
			} else if (functionName.equals("not")) {
				assert appTerm.getParameters().length == 1;
				final Term notnotParam = appTerm.getParameters()[0];
				return notnotParam;
			} else if (functionName.equals("=>")) {
				return SmtUtils.and(script, negateLast(script, params, logger));
			} else if (functionName.equals("=") && SmtUtils.firstParamIsBool(appTerm)) {
				final Term[] notParams = appTerm.getParameters();
				if (notParams.length > 2) {
					final Term binarized = SmtUtils.binarize(script, appTerm);
					return SmtUtils.not(script, binarized, logger);
				} else {
					assert notParams.length == 2;
					return SmtUtils.binaryBooleanNotEquals(script, notParams[0], notParams[1], logger);
				}
			} else if (isXor(appTerm, functionName, logger)) {
				final Term[] notParams = appTerm.getParameters();
				if (notParams.length > 2) {
					...
					// the argument of this call might have been simplified
					// to a term whose function symbol is neither "and" nor "or"
					return SmtUtils.binaryBooleanEquality(script, notParams[0], notParams[1], logger);
				}
			} else if (functionName.equals("ite") && SmtUtils.allParamsAreBool(appTerm)) {
				final Term[] notParams = appTerm.getParameters();
				assert params.length == 3;
				...
				final Term convertedIte = convertIte(script, condTerm, ifTerm, elseTerm, logger);
				return SmtUtils.not(script, convertedIte, logger);
			} else {
			...
			switch (quantifierHandling) {
				...
				case KEEP: {
					...
					final Term innerNegated = SmtUtils.not(script, inner, logger);
