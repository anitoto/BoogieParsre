...
public class DnfTransformer extends XnfTransformer {
	public DnfTransformer(final ManagedScript script, final Logger logger, final Storage storage) {
		this(script, a -> false, logger, storage);
	}

	public DnfTransformer(final ManagedScript script, final Function<Integer, Boolean> funAbortIfExponential,
			final Logger logger, final Storage storage) {
		super(script, false, funAbortIfExponential, logger, storage);
	}

	@Override
	protected NnfTransformerHelper getNnfTransformerHelper(final Logger logger) {
		return new DnfTransformerHelper(logger);
	}

	protected class DnfTransformerHelper extends XnfTransformerHelper {

		protected DnfTransformerHelper(final Logger logger) {
			super(logger);
		}
