...
import ultimate.util.smt.linearterms.BinaryRelation.NoRelationOfThisKindException;
...
public class ArrayUpdate {
	...
	public ArrayUpdate(final Term term, final boolean isNegated, final boolean enforceThatOldArrayIsTermVariable,
			final Logger logger) throws ArrayUpdateException {
		...
		mMultiDimensionalStore = MultiDimensionalStore.convert(allegedStoreTerm, logger);
	...
	public static class ArrayUpdateExtractor {
		 ...
		 * (not (= a (store a' b c)))
		 */
		public ArrayUpdateExtractor(final Logger logger, final boolean negatedUpdate,
				final boolean oldArrayIsTermVariable, final Term... terms) {
			for (final Term term : terms) {
				ArrayUpdate au;
				try {
					au = new ArrayUpdate(term, negatedUpdate, oldArrayIsTermVariable, logger);
	...
	/**
	 * Extract and wrap subterms that are array updates. Enforce that the right hand
	 * side is a TermVariable.
	 * 
	 * @param formula
	 * @return
	 */
	public static List<ArrayUpdate> extractArrayUpdates(final Term formula, final Logger logger) {
		return extractArrayUpdates(formula, true, logger);
	}

	public static List<ArrayUpdate> extractArrayUpdates(final Term formula,
			final boolean enforceThatOldArrayIsTermVariable, final Logger logger) {
		...
		for (final ApplicationTerm subterm : atf.findMatchingSubterms(formula)) {
			...
			try {
				arrayUpdate = new ArrayUpdate(subterm, isNegated, enforceThatOldArrayIsTermVariable, logger);
			} catch (final ArrayUpdateException e) {
				continue;
			}
