...
public class MultiDimensionalSelect {
	 ...
	 *             index i2.
	 */
	public MultiDimensionalSelect(Term term, final Logger logger) {
		this.logger = logger;
	...
	public MultiDimensionalSelect(final Term array, final ArrayIndex index, final Script script, final Logger logger) {
		super();
		this.logger = logger;
		mArray = array;
		mIndex = index;
	...
	public MultiDimensionalSelect getInnermost(final int dim) {
		...
		return MultiDimensionalSelect.convert(as.asTerm(), logger);
	}
	...
	private boolean classInvariant() {
		if (mArray == null) {
			...
			return MultiDimensionalSort.areDimensionsConsistent(mArray, mIndex, mSelectTerm);
		}
	}
	...
	public static MultiDimensionalSelect convert(final Term term, final Logger logger) {
		if (!(term instanceof ApplicationTerm)) {
			return null;
		}
		final MultiDimensionalSelect mds = new MultiDimensionalSelect(term, logger);
	 ...
	 *                         Sort is an array Sort (these select terms occur
	 *                         usually in
	 *                         multidimensional store terms).
	 */
	public static List<MultiDimensionalSelect> extractSelectShallow(final Term term, final boolean allowArrayValues,
			final Logger logger) {
		...
		for (final Term storeTerm : selectTerms) {
			if (allowArrayValues || !storeTerm.getSort().isArraySort()) {
				final MultiDimensionalSelect mdSelect = new MultiDimensionalSelect(storeTerm, logger);
	 ...
	 *                         multidimensional store terms).
	 */
	public static List<MultiDimensionalSelect> extractSelectDeep(final Term term, final boolean allowArrayValues,
			final Logger logger) {
		final List<MultiDimensionalSelect> result = new LinkedList<MultiDimensionalSelect>();
		List<MultiDimensionalSelect> foundInThisIteration = extractSelectShallow(term, allowArrayValues, logger);
		while (!foundInThisIteration.isEmpty()) {
			...
			for (final MultiDimensionalSelect mdSelect : foundInLastIteration) {
				foundInThisIteration.addAll(extractSelectShallow(mdSelect.getArray(), allowArrayValues, logger));
				final ArrayIndex index = mdSelect.getIndex();
				for (final Term entry : index) {
					foundInThisIteration.addAll(extractSelectShallow(entry, allowArrayValues, logger));
				}
			}
		}
		return result;
