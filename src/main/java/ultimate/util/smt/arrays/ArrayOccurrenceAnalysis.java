...
public class ArrayOccurrenceAnalysis {
	...
	public ArrayOccurrenceAnalysis(final Script script, final Term analyzedTerm, final Term wantedArray,
			final int dimensionUpperLimit, final Logger logger) {
		super();
		this.logger = logger;
		mAnalyzedTerm = analyzedTerm;
		mWantedArray = wantedArray;
	...
	/**
	 * @return from the analyzed term all select-over-store subterms whose array is
	 *         the wantedArray
	 */
	...
	/**
	 * @return from the analyzed term all (possibly nested) store subterms whose
	 *         array is the wantedArray
	 *         such that the store subterms are not part of a select-over-store
	 *         subterm.
	 */
	...
	/**
	 * @return from the analyzed term all select subterms whose array is the
	 *         wantedArray
	 */
	...
	public ArrayOccurrenceAnalysis downgradeDimensionsIfNecessary(final Script script) {
		...
		if (dims.size() <= 1) {
			...
			return new ArrayOccurrenceAnalysis(script, mAnalyzedTerm, mWantedArray, dimensionUpperLimit, logger);
		}
	}
		...
		class MyWalker extends TermWalker {
			...
			public void walk(final NonRecursive walker, final ApplicationTerm term) {
				...
				if (fun.equals("=")) {
					...
					MultiDimensionalNestedStore nas = MultiDimensionalNestedStore.convert(mScript, term, logger);
					if (nas != null && nas.getArray().equals(mWantedArray)) {
						...
						if (nas.getDimension() > mDimensionUpperLimit) {
							...
							nas = new MultiDimensionalNestedStore(MultiDimensionalStore
									.convert(nas.getInnermost(1).getStoreTerm(), mDimensionUpperLimit, logger), logger);
					...
					if (asos != null) {
						...
						MultiDimensionalSelect as = MultiDimensionalSelect.convert(term, logger);
