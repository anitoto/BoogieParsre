...
public class MultiDimensionalStore {
	...
	public MultiDimensionalStore(final Term array, final ArrayIndex index, final Term value, final Script script, final Logger logger) {
		super();
		this.logger = logger;
		mArray = array;
		mIndex = index;
	...
	public MultiDimensionalStore(final Term array, final ArrayIndex index, final Term value, final Term asTerm, final Logger logger) {
		super();
		this.logger = logger;
		mArray = array;
		mIndex = index;
	...
	public MultiDimensionalStore(final Term term, final Logger logger) {
		this.logger = logger;
		...
		if (isStore(term)) {
			...
			while (isStore(remainder) && isCompatibleSelect(((ApplicationTerm) remainder).getParameters()[0], mArray, index, logger)) {
	...
	static boolean isCompatibleSelect(final Term term, final Term array, final List<Term> index, final Logger logger) {
		final MultiDimensionalSelect mdSelect = new MultiDimensionalSelect(term, logger);
	...
	public MultiDimensionalStore getInnermost(final int dim) {
		...
		return MultiDimensionalStore.convert(as.asTerm(), logger);
	}

	public static MultiDimensionalStore convert(final Term term, final Logger logger) {
		return convert(term, Integer.MAX_VALUE, logger);
	}

	public static MultiDimensionalStore convert(final Term term, final int maxDimension, final Logger logger) {
		final ArrayList<Term> index = new ArrayList<Term>();
		Term remainder = term;
		final Term array;
		if (isStore(term)) {
			...
			while (dimension < maxDimension && isStore(remainder)
					&& isCompatibleSelect(((ApplicationTerm) remainder).getParameters()[0], array, index, logger)) {
				index.add(((ApplicationTerm) remainder).getParameters()[1]);
				remainder = ((ApplicationTerm) remainder).getParameters()[2];
				dimension ++;
			}
		...
		return new MultiDimensionalStore(array, new ArrayIndex(index), remainder, term, logger);
	}
	 ...
	 * in the result.
	 */
	public static List<MultiDimensionalStore> extractArrayStoresShallow(final Term term, final Logger logger) {
		...
		for (final Term storeTerm : storeTerms) {
			final MultiDimensionalStore mdStore = MultiDimensionalStore.convert(storeTerm, logger);
	 ...
	 */
	public static List<MultiDimensionalStore> extractArrayStoresDeep(final Term term, final Logger logger) {
		final List<MultiDimensionalStore> result = new LinkedList<MultiDimensionalStore>();
		List<MultiDimensionalStore> foundInThisIteration = extractArrayStoresShallow(term, logger);
		while (!foundInThisIteration.isEmpty()) {
			...
			for (final MultiDimensionalStore asd : foundInLastIteration) {
				foundInThisIteration.addAll(extractArrayStoresShallow(asd.getArray(), logger));
				foundInThisIteration.addAll(extractArrayStoresShallow(asd.getValue(), logger));
				final ArrayIndex index = asd.getIndex();
				for (final Term entry : index) {
					foundInThisIteration.addAll(extractArrayStoresShallow(entry, logger));
				}
			}
		}
		return result;
