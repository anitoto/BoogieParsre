...
import ultimate.util.smt.linearterms.BinaryEqualityRelation;
...
public class DerPreprocessor extends TermTransformer {
	...
	public DerPreprocessor(final ManagedScript mgdScript, final int quantifier, final TermVariable eliminatee,
			final Term input, final List<BinaryEqualityRelation> bers, final ArrayIndexEqualityManager aiem,
			final Logger logger) {
		...
		final ArrayIndexReplacementConstructor airc = new ArrayIndexReplacementConstructor(mgdScript, AUX_VAR_PREFIX,
				eliminatee);
		...
		if (someTopLevelEquality != null) {
			...
			substitutionMapping = handleAllSelfUpdates(classification.getImage(DerCase.SELF_UPDATE), mgdScript,
					eliminatee, quantifier, airc, aiem, logger);
			mIntroducedDerPossibility = false;
		}
		final Term inputReplacement = new SubstitutionWithLocalSimplification(mgdScript, substitutionMapping, logger)
				.transform(input);
		final Term allAuxVarDefs = airc.constructDefinitions(mgdScript.getScript(), quantifier, logger);
	...
	private static Map<Term, Term> handleAllSelfUpdates(final Set<BinaryEqualityRelation> selfupdates,
			...
			final ArrayIndexReplacementConstructor airc, final ArrayIndexEqualityManager aiem, final Logger logger) {
	...
	private static Term constructDerEnabler(final BinaryEqualityRelation someTopLevelEquality,
		...
		switch (derCase) {
			case EQ_SELECT:
				final MultiDimensionalSelect as = MultiDimensionalSelect.convert(otherSide, logger);
				result = constructReplacementForSelectCase(as.getArray(), as.getIndex(), mgdScript, eliminatee,
						quantifier, airc, logger);
				break;
			case EQ_STORE:
				final MultiDimensionalNestedStore nas = MultiDimensionalNestedStore.convert(mgdScript.getScript(),
						otherSide, logger);
				result = constructReplacementForStoreCase(nas, mgdScript, eliminatee, quantifier, airc, aiem, logger);
				break;
			case SELF_UPDATE:
			default:
	...
	private static HashRelation<DerCase, BinaryEqualityRelation> classify(final Script script,
			final List<BinaryEqualityRelation> bers, final TermVariable eliminatee, final Logger logger) {
		...
		for (final BinaryEqualityRelation ber : bers) {
			...
			final DerCase derCase = classify(script, otherSide, eliminatee, logger);
	...
	private static DerCase classify(final Script script, final Term otherSide, final TermVariable eliminatee,
		...
		final MultiDimensionalNestedStore mdns = MultiDimensionalNestedStore.convert(script, otherSide, logger);
		if (mdns != null) {
		...
		final MultiDimensionalSelect arraySelect = MultiDimensionalSelect.convert(otherSide, logger);
		if (arraySelect != null) {
		...
		if (nas.getArray().equals(eliminatee)) {
			...
			for (int i = 0; i < newIndices.size(); i++) {
				...
				resultDualFiniteJuncts[i] = constructDisjointIndexImplication(innermostIndex, indices, innermostValue,
						eliminatee, mgdScript.getScript(), quantifier, aiem, logger);
			}
			...
			result = QuantifierUtils.applyDerOperator(mgdScript.getScript(), quantifier,
					new MultiDimensionalNestedStore(mgdScript.getScript(), nas.getArray(), newIndices, newValues,
							logger).toTerm(mgdScript.getScript()),
					eliminatee, logger);
		}
		return result;
	}
	...
	private static Term constructReplacementForSelectCase(final Term array, final ArrayIndex arrayIndex,
			final ManagedScript mgdScript, final TermVariable eliminatee, final int quantifier,
			final ArrayIndexReplacementConstructor airc, final Logger logger) {
		...
		final MultiDimensionalSelect mds = new MultiDimensionalSelect(array, newIndex, mgdScript.getScript(), logger);
		final Term result = QuantifierUtils.applyDerOperator(mgdScript.getScript(), quantifier, eliminatee,
				mds.toTerm(mgdScript.getScript()), logger);
		return result;
	}
	...
	private static Term constructDisjointIndexImplication(final ArrayIndex innermostIndex,
			...
			final int quantifier, final ArrayIndexEqualityManager aiem, final Logger logger) {
		final Term select = new MultiDimensionalSelect(arr, innermostIndex, script, logger).toTerm(script);
