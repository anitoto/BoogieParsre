...
/**
 * Transform term into an equivalent term without ite terms. E.g., a = b ? c : d
 * becomes b && a = c || !b && a = d (= a
 * (ite b c d)) becomes (or (and b (= a c)) (and (not b) (= a d)))
 *
 * TODO: This could be much more efficient if could obtain the context in which
 * the ite term occurs and check if the
 * condition holds.(Use ideas from SimplifyDDA). For small formulas we can
 * achieve the same result by applying
...
public class IteRemover extends NonCoreBooleanSubTermTransformer {
	...
	public IteRemover(final ManagedScript script, final Logger logger) {
		super();
		this.logger = logger;
		mScript = script;
	}
	...
	private Term removeIteTerm(final Term term, final ApplicationTerm iteTerm) {
		...
		Term replacedWithIf;
		{
			...
			replacedWithIf = (new SubstitutionWithLocalSimplification(mScript, substitutionMapping, logger))
					.transform(term);
		}
		Term replacedWithElse;
		{
			final Map<Term, Term> substitutionMapping = Collections.singletonMap((Term) iteTerm, elseTerm);
			replacedWithElse = (new SubstitutionWithLocalSimplification(mScript, substitutionMapping, logger))
					.transform(term);
		}
		final Term withoutThisIte = SmtUtils.or(mScript.getScript(),
				SmtUtils.and(mScript.getScript(), condition, replacedWithIf),
				SmtUtils.and(mScript.getScript(), SmtUtils.not(mScript.getScript(), condition, logger), replacedWithElse));
