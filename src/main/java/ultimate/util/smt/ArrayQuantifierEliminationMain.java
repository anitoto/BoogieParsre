...
import ultimate.util.smt.arrays.NestedArrayStore;
...
public class ArrayQuantifierEliminationMain {
	 ...
	 */
	public ArrayQuantifierEliminationMain(final ManagedScript mgdScript,
	...
	public EliminationTask elimAllRec(final EliminationTask eTask) {
		...
		logger.info("ArrayQuantifierEliminationMain: ");
		logger.fine("Recursion:" + mRecursiveCallCounter + " Eliminatees: " + mEliminatees);
		mRecursiveCallCounter += 1;
		logger.fine("Term: " + eTask.getTerm());
		...
		logger.fine("Result term: " + recTask.getTerm());
	...
	private EliminationTask recursivCall(final EliminationTask eTask, final Term quantifiedTerm) {
		...
		final Term nnf = new NnfTransformer(mMgdScript, QuantifierHandling.KEEP, logger).transform(quantifiedTerm);
		final Term pushed = new QuantifierPusher(mMgdScript, true, PqeTechniques.ALL_LOCAL, logger, storage)
				.transform(nnf);
		final Term pnf = new PrenexNormalForm(mMgdScript, logger).transform(pushed);
		...
		for (int i = qvs.size() - 1; i >= 0; i--) {
			...
			if (!eliminatees.equals(eTask.getEliminatees())) {
				matrix = SmtUtils.simplify(mMgdScript, matrix, mSimplificationTechnique, logger, storage);
				...
				matrix = new QuantifierPusher(mMgdScript, true, PqeTechniques.ONLY_DER, logger, storage)
						.transform(matrix);
			}
		}
	...
	private Term storeOverStore(final Term term, final TermVariable qarray) {
		final List<MultiDimensionalStore> storeterms = MultiDimensionalStore.extractArrayStoresShallow(term, logger);
			...
			if (nas.getIndices().size() > 1) {
				...
				if ((qarray.equals(nas.getArray()))
					...
					final Substitution sub = new Substitution(mMgdScript,
							Collections.singletonMap(storeOuter.getStoreTerm(), newStore), logger);
				...
				final Term innerStore = SmtUtils.store(mScript, nas.getArray(), nas.getIndices().get(0),
						nas.getValues().get(0));
				final Substitution sub = new Substitution(mMgdScript,
						Collections.singletonMap(innerStore, newarrayvar), logger);

				final Term noSOSterm = sub.transform(term);
				mNewExistEliminatees.add(newarrayvar);
				// Index comparison Equality
				final Term factorOutStore = SmtUtils.binaryEquality(mScript, newarrayvar, innerStore, logger);
	...
	private Term indexEQ(final Term index, final MultiDimensionalSelect select) {
		Term indexeq = SmtUtils.binaryEquality(mScript, index, select.getIndex().get(0), logger);
	...
	private Term selectOverStore(final Term term, final TermVariable qarray) {
		final List<MultiDimensionalSelect> selectterms = MultiDimensionalSelect.extractSelectDeep(term, false, logger);
		for (final MultiDimensionalSelect select : selectterms) {
			...
			if (!SmtUtils.isBasicArrayTerm(select.getArray())) {
				final MultiDimensionalStore innerStore = MultiDimensionalStore.convert(select.getArray(), logger);
				...
				if (qarray.equals(nas.getArray())) {
					...
					for (int i = nas.getIndices().size() - 1; i >= 0; i--) {
						...
						final Term indexnoteq = SmtUtils.not(mScript, indexeq, logger);

						final Substitution sub = new Substitution(mMgdScript, Collections.singletonMap(
								select.getSelectTerm(), nas.getValues().get(nas.getIndices().indexOf(index))), logger);
	...
	private Term indexEQvalueEQ(final Pair<ApplicationTerm, ApplicationTerm> comb, final TermVariable si,
		...
		final Term lhs = SmtUtils.binaryEquality(mScript, comb.getFirst().getParameters()[1],
				comb.getSecond().getParameters()[1], logger);
		final Term rhs = SmtUtils.binaryEquality(mScript, si, sj, logger);

		Term iEvE = SmtUtils.or(mScript, SmtUtils.not(mScript, lhs, logger), rhs);
		if (quantifier == QuantifiedFormula.FORALL) {
			iEvE = SmtUtils.not(mScript, iEvE, logger);
		}
		return iEvE;
	}
	...
	public EliminationTask elimArrayBySelects(final Term eTerm, final TermVariable eliminate, final int quantifier) {
		...
		if (!indexCombinations.isEmpty()) {
			...
			final Substitution sub = new Substitution(mMgdScript, submap, logger);
	...
	private Term elimStoreEQ(final TermVariable newindexvar, final TermVariable newarrayvar,
		...
		final Term indexnoteq = SmtUtils.not(mScript,
				SmtUtils.binaryEquality(mScript, newindexvar, store.getParameters()[1], logger), logger);
		final Term arrayeq = SmtUtils.binaryEquality(mScript, SmtUtils.select(mScript, newarrayvar, newindexvar),
				SmtUtils.select(mScript, store.getParameters()[0], newindexvar), logger);
		final Term elimtermlhs = SmtUtils.implies(mScript, indexnoteq, arrayeq, logger);
		// Term 2: ((i = j) => (a_new[i] = v))
		final Term indexeq = SmtUtils.binaryEquality(mScript, newindexvar, store.getParameters()[1], logger);
		final Term selectvalue = SmtUtils.binaryEquality(mScript,
				SmtUtils.select(mScript, newarrayvar, store.getParameters()[1]), store.getParameters()[2], logger);
		final Term elimtermrhs = SmtUtils.implies(mScript, indexeq, selectvalue, logger);
		...
		final Substitution sub = new Substitution(mMgdScript, Collections.singletonMap(storeEQ, elimterm), logger);
	...
	private EliminationTask elimStores(final EliminationTask eTask, final TermVariable qarray) {
		...
		for (final ApplicationTerm term : storeterms) {
			if (qarray.equals(term.getParameters()[0])) {
				...
				final Term eqterm = SmtUtils.binaryEquality(mScript, newarrayvar, term, logger);
