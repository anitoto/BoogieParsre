...
public final class UltimateNormalFormUtils {
	...
	private static boolean rootRespectsUltimateNormalForm(final ConstantTerm term) {
		if (SmtSortUtils.isBitvecSort(term.getSort())) {
			assert false
					: "UNF does not support bitvector literals, use instead e.g., (_ bv23 64) to construct a bitvector of length 64 whose decimal value is 23.";
			return false;
		}
	...
	private static boolean rootRespectsUltimateNormalForm(final ApplicationTerm term, final Logger logger) {
	...
	/**
	 * Check if a term respects a certain normal form that we want to enforce in
	 * Ultimate in order to be able to
	 * simplify terms more easily.
	 * <ul>
	 * <li>Values of {@link ConstantTerm}s have to be represented by
	 * {@link Rationals} (instead of {@link BigInteger}
	 * and {@link BigDecimal}).
	 * <li>Do not use "distinct" terms. Always use negated equalities instead. This
	 * allows us to detect that (and (= a
