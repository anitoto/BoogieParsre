...
import ultimate.smtinterpol.smtlib2.SMTInterpol;
import ultimate.smtinterpol.smtlib2.TerminationRequest;
import ultimate.smtsolver.NonIncrementalScriptor;
import ultimate.smtsolver.Scriptor;
import ultimate.smtsolver.ScriptorWithGetInterpolants;
import ultimate.smtsolver.ScriptorWithGetInterpolants.ExternalInterpolator;
import ultimate.smtsolver.SmtInterpolLogProxyWrapper;
import ultimate.util.CoreUtil;
import ultimate.util.smt.arrays.DiffWrapperScript;
...
public class SolverBuilder {
	...
	private static Script createSMTInterpol(final Logger logger, final Storage storage) {
		final LogProxy loggerWrapper = new SmtInterpolLogProxyWrapper(logger);
		final TerminationRequest termRequest = new SMTInterpolTerminationRequest(storage.getProgressMonitorService());
		return new SMTInterpol(loggerWrapper, termRequest);
	}

	private static Script createExternalSolver(final String command, final boolean fakeNonIncrementalScript,
			final boolean dumpFakeNonIncrementalScript, final String pathOfDumpedFakeNonIncrementalScript,
			final String basenameOfDumpedFakeNonIcrementalScript, final boolean useDiffWrapper,
			final boolean omitLogicOutput, final Logger logger, final Storage storage) throws IOException {
		Script script;
		if (fakeNonIncrementalScript) {
			script = new NonIncrementalScriptor(command, "External_FakeNonIncremental", dumpFakeNonIncrementalScript,
					pathOfDumpedFakeNonIncrementalScript, basenameOfDumpedFakeNonIcrementalScript, logger, storage);
		} else {
			script = new Scriptor(command, "External", omitLogicOutput, logger, storage);
		}
		if (useDiffWrapper) {
	...
	private static Script createExternalSolverWithInterpolation(final String command,
			final ExternalInterpolator externalInterpolator, final boolean useDiffWrapper, final Logger logger,
			final Storage storage) throws IOException {
		Script script = new ScriptorWithGetInterpolants(command, externalInterpolator, "ExternalInterpolator", logger,
				storage);
		if (useDiffWrapper) {
			script = new DiffWrapperScript(script);
		}
	 ...
	 */
	public static Script buildScript(final SolverSettings settings, final Logger logger, final Storage storage) {
		Script result;
		if (settings.useExternalSolver()) {
			logger.info("constructing external solver with command " + settings.getCommandExternalSolver());
			try {
				if (settings.getExternalInterpolator() == null) {
					result = createExternalSolver(settings.getCommandExternalSolver(),
							...
							settings.getUseDiffWrapper(), settings.omitLogicOutput(), logger, storage);
				} else {
					logger.info(
							"external solver will use " + settings.getExternalInterpolator() + " interpolation mode");
					result = createExternalSolverWithInterpolation(settings.getCommandExternalSolver(),
							settings.getExternalInterpolator(), settings.getUseDiffWrapper(), logger, storage);
				}
			} catch (final IOException e) {
				logger.severe("Unable to construct solver");
				throw new RuntimeException(e);
			}
		} else {
			logger.info("constructing new instance of SMTInterpol");
			result = createSMTInterpol(logger, storage);
		}
		if (settings.dumpSmtScriptToFile()) {
			try {
				result = new LoggingScript(result, settings.constructFullPathOfDumpedScript(), true);
				logger.info("Dumping SMT script to " + settings.constructFullPathOfDumpedScript());
			} catch (final FileNotFoundException e) {
				logger.severe("Unable dump SMT script to " + settings.constructFullPathOfDumpedScript());
				throw new RuntimeException(e);
			}
		}
		if (!settings.useExternalSolver()) {
	...
	public static SolverSettings constructSolverSettings(final String filename, final SolverMode solverMode,
		...
		return constructSolverSettings(filename, solverMode, fakeNonIncrementalScript, commandExternalSolver,
				dumpSmtScriptToFile,
	...
	public static SolverSettings constructSolverSettings(final String filename, final SolverMode solverMode,
			final boolean fakeNonIncrementalScript, final String commandExternalSolver,
			final boolean dumpSmtScriptToFile, final String pathOfDumpedScript, boolean omitLogicOutput)
		...
		final SolverSettings solverSettings = new SolverSettings(fakeNonIncrementalScript, useExternalSolver,
				commandExternalSolver, timeoutSmtInterpol,
				externalInterpolator, dumpSmtScriptToFile, pathOfDumpedScript, filename, useDiffWrapper,
				omitLogicOutput);
		return solverSettings;
	}

	public static Script buildAndInitializeSolver(final SolverMode solverMode, final SolverSettings solverSettings,
			final boolean dumpUsatCoreTrackBenchmark, final boolean dumpMainTrackBenchmark,
			final String logicForExternalSolver, final String solverId, final Logger logger, final Storage storage)
			throws AssertionError {

		Script script = SolverBuilder.buildScript(solverSettings, logger, storage);
		...
		switch (solverMode) {
			...
			case External_Z3InterpolationMode:
				...
				if (logicForExternalSolver.endsWith("A")) {
					...
					// result.declareFun("array-ext", new Sort[] { boolArraySort, boolArraySort },
					// indexSort);
	...
	public static final class SolverSettings {
		...
		/**
		 * Emulate incremental script (push/pop) using reset and re-asserting all terms
		 * and re-declaring all sorts and
		 * functions.
		 */
		...
		/**
		 * Base name (without path and without file ending) of the file to which the SMT
		 * solver script is written.
		 */
		...
		public SolverSettings(final boolean fakeNonIncrementalScript, final boolean useExternalSolver,
			...
			this(fakeNonIncrementalScript, useExternalSolver, commandExternalSolver, timeoutSmtInterpol,
					externalInterpolator, dumpSmtScriptToFile, pathOfDumpedScript, baseNameOfDumpedScript,
					useDiffWrapper,
					false);
		}
		...
		/**
		 * Check whether to use the diff wrapper script to add support for the diff
		 * function.
		 */
