...
import ultimate.util.smt.IncrementalPlicationChecker.Plication;
...
public class ArrayIndexEqualityManager {
	...
	public ArrayIndexEqualityManager(final ThreeValuedEquivalenceRelation<Term> tver, final Term context,
			final int quantifier, final Logger logger, final ManagedScript mgdScript) {
		super();
		mTver = tver;
		mContext = context;
		...
		mIea = new IncrementalPlicationChecker(plication, mMgdScript, mContext, logger);
	...
	private void checkEqualityStatusViaSolver(final int mQuantifier, final ThreeValuedEquivalenceRelation<Term> tver,
		...
		final Term eq = SmtUtils.binaryEquality(mMgdScript.getScript(), index1, index2, logger);
		if (SmtUtils.isTrue(eq)) {
			...
			final Term neq = SmtUtils.not(mMgdScript.getScript(), eq, logger);
			final Validity isEqual = iea.checkPlication(eq);
			if (isEqual == Validity.UNKNOWN) {
				logger.warning("solver failed to check if following equality is implied: " + eq);
			}
			if (isEqual == Validity.VALID) {
				...
				logger.info("detected equality via solver");
			} else {
				final Validity notEqualsHolds = iea.checkPlication(neq);
				if (notEqualsHolds == Validity.UNKNOWN) {
					logger.warning("solver failed to check if following not equals relation is implied: " + eq);
				}
				...
				if (notEqualsHolds == Validity.VALID) {
					...
					logger.info("detected not equals via solver");
				}
			}
		}
	}
	...
	public Term constructIndexEquality(final ArrayIndex index1, final ArrayIndex index2) {
		...
		for (int i = 0; i < index1.size(); i++) {
			...
			switch (indexEquality) {
				...
				case UNKNOWN:
					conjuncts
							.add(SmtUtils.binaryEquality(mMgdScript.getScript(), index1.get(i), index2.get(i), logger));
					break;
				default:
	...
	public Term constructDerRelation(final Script script, final int quantifier, final Term t1, final Term t2) {
		...
		switch (eq) {
			...
			case NOT_EQUAL:
				...
				break;
			case UNKNOWN:
				result = QuantifierUtils.applyDerOperator(script, quantifier, t1, t2, logger);
				break;
			default:
				throw new AssertionError();
		}
	...
	public Term constructDerRelation(final Script script, final int quantifier, final ArrayIndex idx1,
		...
		for (int i = 0; i < idx1.size(); i++) {
			...
			switch (entryEquality) {
				...
				case UNKNOWN:
					dualJuncts.add(
							QuantifierUtils.applyDerOperator(script, quantifier, idx1.get(i), idx2.get(i), logger));
					break;
				default:
					throw new AssertionError();
			}
	...
	public Term constructAntiDerRelation(final Script script, final int quantifier, final ArrayIndex idx1,
		...
		for (int i = 0; i < idx1.size(); i++) {
			...
			switch (entryEquality) {
				...
				case UNKNOWN:
					sameJuncts.add(
							QuantifierUtils.applyAntiDerOperator(script, quantifier, idx1.get(i), idx2.get(i), logger));
					break;
				default:
					throw new AssertionError();
			}
	 ...
	 *                           indices that occur later in the nested stores,
	 *                           outermost at the
	 *                           last position
	 */
	private Term constructNestedStoreUpdateConstraintForOnePosition(final Script script, final int quantifier,
		...
		final MultiDimensionalSelect idxCellOfArrayRes = new MultiDimensionalSelect(arrayRes, idx, script, logger);
