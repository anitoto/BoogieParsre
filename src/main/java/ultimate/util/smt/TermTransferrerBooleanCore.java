...
/**
 * Variant of TermTransferrer that transfers only the boolean structure of a
 * term. Each relation of non-boolean
 * arguments (i.e., literals in the boolean structure) is transferred to a fresh
 * TermVariable. QuantifiedFormulas are
 * always considered as a literal even if they only quantify boolean Variables
...
public class TermTransferrerBooleanCore extends TermTransferrer {
	...
	/**
	 * Auxiliary term that is used in the "recurse phase" to mark non-boolean
	 * subterms. Later, in the
	 * "construction phase" we identify literals as the {@link ApplicationTerm}s
	 * that have an argument that was replaced
	...
	public TermTransferrerBooleanCore(final Script script, final Logger logger) {
		super(script, logger);
