...
import ultimate.models.Payload;
 ...
 * which are both ProgramPoints and define
 * how the program counter is modified while executing this CodeBlock.
 * Furthermore the subclasses of a CodeBlock define
 * how variables of the program are manipulated while executing this CodeBlock.
 * A CodeBlock is either
 * <ul>
 * <li>a sequence of Statements where each Statement is either an
 * AssumeStatement, an AssignmentStatement, or a Havoc
 * statement.
 ...
 * the construction of the control flow
 * graph is complete.
 *
 * In an ULTIMATE graph a CodeBlock is an edge as well as an annotation of this
 * edge.
 *
 * mTransitionFormula stores a TransitionFormula that describes the effect of
 * this InternalEdge. (TODO: Add this
 * information later, as additional annotation)
 *
 * mOccurenceInCounterexamples is used to store in a CEGAR based verification
 * process how often this CodeBlock occurred
 * in a counterexample. (TODO: Store this information somewhere in the model
 * checker)
 *
...
public abstract class CodeBlock extends IcfgEdge {
	 ...
	 * additional fields, this constant should be
	...
	/**
	 * This constructor is for subclasses that are not constructed by the
	 * CodeBlockFactory. All these CodeBlocks will
	 * have serial number "-1" and hence they will have the same hash code.
	 *
	 * @deprecated Do not use this constructor, use the {@link CodeBlockFactory}
	 *             instead.
	 */
	@Deprecated
	public CodeBlock(final BoogieIcfgLocation source, final BoogieIcfgLocation target, final Logger logger) {
		super(source, target, source == null ? new Payload() : new Payload(source.getPayload()));
		mSerialnumber = -1;
		this.logger = logger;
		connectSource(source);
	...
	/**
	 * Returns the procedure in that the system was before executing this CodeBlock.
	 * E.g., if CodeBlock is a call, the
	 * result is the name of the caller, if CodeBlock is a return the result is the
	 * callee (from which we return).
	 */
	@Override
	 ...
	 * CodeBlock. E.g., if CodeBlock is a call, the
	 * result is the name of the callee, if CodeBlock is a return the result is the
	 * caller (to which we return).
	 */
	@Override
