 ...
 * called procedure. This represents the
 * execution starting from the position directly before the return statement
 * (resp. the last position of a procedure if
 * there is no return statement) to the position after the corresponding call
 * statement. The update of the variables of
...
public class Return extends CodeBlock implements IIcfgReturnTransition<IcfgLocation, Call> {
	...
	Return(final int serialNumber, final BoogieIcfgLocation source, final BoogieIcfgLocation target,
			final Call correspondingCall, final Logger logger) {
