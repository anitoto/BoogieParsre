...
public class ParallelComposition extends CodeBlock implements IIcfgInternalTransition<IcfgLocation> {
	...
	ParallelComposition(final int serialNumber, final BoogieIcfgLocation source, final BoogieIcfgLocation target,
			final ManagedScript mgdScript, final List<CodeBlock> codeBlocks,
		...
		final UnmodifiableTransFormula[] transFormulasWithBranchEncoders = new UnmodifiableTransFormula[codeBlocks
				.size()];
		...
		final boolean s_TransformToCNF = AppConfig.cnf.get();

		mTransitionFormula = TransFormulaUtils.parallelComposition(logger, storage, getSerialNumber(), mgdScript, null,
				s_TransformToCNF, xnfConversionTechnique, transFormulas);
		mTransitionFormulaWithBranchEncoders = TransFormulaUtils.parallelComposition(logger, storage, getSerialNumber(),
				mgdScript, branchIndicator, s_TransformToCNF, xnfConversionTechnique, transFormulasWithBranchEncoders);
	}
