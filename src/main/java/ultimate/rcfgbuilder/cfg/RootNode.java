...
import ultimate.util.cfg.structure.debugidentifiers.ArtificialRootDebugIdentifier;
...
public class RootNode extends IcfgLocation {
	...
	public RootNode(final ILocation location, final BoogieIcfgContainer rootAnnot) {
		...
		getPayload().getAnnotations().put("RCFGBuilder", rootAnnot);
		location.annotate(this);
	}

	@Deprecated
	public BoogieIcfgContainer getRootAnnot() {
		return (BoogieIcfgContainer) getPayload().getAnnotations().get("RCFGBuilder");
	}
