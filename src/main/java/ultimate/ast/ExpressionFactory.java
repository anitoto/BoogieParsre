...
import ultimate.ast.BinaryExpression.Operator;
...
import ultimate.ast.StructAccessExpression;
import ultimate.ast.StructConstructor;
import ultimate.ast.StructLHS;
import ultimate.ast.UnaryExpression;
import ultimate.ast.VariableLHS;
import ultimate.ast.WildcardExpression;
...
import ultimate.typechecker.TypeCheckHelper;
import ultimate.util.BoogieUtils;
import ultimate.util.datastructures.BitvectorConstant;
import ultimate.util.datastructures.BitvectorConstant.BitvectorConstantOperationResult;
import ultimate.util.datastructures.BitvectorConstant.SupportedBitvectorOperations;

/**
 * Construct Boogie Expressions (and LeftHandSides), use this instead of the
 * constructors. Functionalities:
 * <ul>
 * <li>do minor simplifications (e.g. if all operands are literals)
 * <li>computes types for the resulting Boogie expressions, throws an exception
 * if it cannot be typed. Note that this
 * means that all input Boogie expressions must have a type. (Which is the case
 * if they have been constructed using this
 * factory.)
 * </ul>
 *
...
public class ExpressionFactory {
	...
	/**
	 * Name for dummy expressions that represent a "void" result. Those identifier
	 * expressions may not be used anywhere
	 * and thus should get an error BoogieType. (note that this string has to fit
	 * the regex that is checked during
	 * creation of an IdentifierExpression...)
	 */
	public static final String DUMMY_VOID = "#dummy~void~value";

	public ExpressionFactory(final Logger logger) {
		this.logger = logger;
	}

	public Expression constructUnaryExpression(
			final ILocation loc,
			final UnaryExpression.Operator operator,
			final Expression expr) {
		final BoogieType resultType = TypeCheckHelper.typeCheckUnaryExpression(
				operator,
				(BoogieType) expr.getType(),
				expr,
				logger);
	...
	public Expression newBinaryExpression(
			final ILocation loc,
			final Operator op,
		...
		if (isLeftLiteral && isRightLiteral) {
			...
			if (right instanceof BinaryExpression) {
				...
				// if expression is of the form (op c1 (op c2 x)), make (op c3 x) with c3 == (op
				// c1 c2)
	...
	private Expression computeBinaryExpression(final ILocation loc, final Operator op, final Expression left,
			final Expression right) {
	...
	private BooleanLiteral constructBinExprWithLiteralOps_Bool(final ILocation loc, final Operator operator,
	...
	private Expression constructBinExprWithLiteralOps_Integer(final ILocation loc, final Operator operator,
	...
	private Expression constructBinExprWithLiteralOps_Real(final ILocation loc, final Operator operator,
	...
	private Expression constructBinExprWithLiteralOps_Bitvector(final ILocation loc, final Operator operator,
	...
	public Expression constructIfThenElseExpression(
			final ILocation loc,
			final Expression condition,
			final Expression thenPart,
			final Expression elsePart) {
		...
		final BoogieType type = TypeCheckHelper.typeCheckIfThenElseExpression(
				(BoogieType) condition.getType(),
				(BoogieType) thenPart.getType(),
				(BoogieType) elsePart.getType(),
				condition, logger);
		return new IfThenElseExpression(loc, type, condition, thenPart, elsePart);

	}

	private boolean isLiteral(final Expression expr) {
	...
	public boolean isTrueLiteral(final Expression expr) {
		if (expr instanceof BooleanLiteral) {
	 ...
	 * @return
	 */
	public Expression constructBitvectorAccessExpression(
			final ILocation loc,
		...
		final BoogieType type = TypeCheckHelper.typeCheckBitVectorAccessExpression(
				TypeCheckHelper.getBitVecLength((BoogieType) operand.getType()),
				high,
				low,
				(BoogieType) operand.getType(),
				operand, logger);
	...
	public Expression and(final ILocation loc, final List<Expression> exprs) {
		return bin(loc, exprs, true, Operator.LOGICAND);
	}

	public Expression or(final ILocation loc, final List<Expression> exprs) {
		return bin(loc, exprs, false, Operator.LOGICOR);
	}

	private Expression bin(final ILocation loc, final List<Expression> exprs, final boolean neutralElement,
			final Operator op) {
	...
	public StructConstructor constructStructConstructor(final ILocation loc, final String[] fieldIds,
	...
	public StructLHS constructStructAccessLhs(
			final ILocation loc,
			final LeftHandSide struct,
			final String fieldName) {
		final BoogieType lhsType = TypeCheckHelper.typeCheckStructAccessExpressionOrLhs((BoogieType) struct.getType(),
				fieldName, struct.toString(), logger);
		return new StructLHS(loc, lhsType, struct, fieldName);
	}

	public ArrayAccessExpression constructNestedArrayAccessExpression(
			final ILocation loc,
		...
		if (indices.length == 1) {
			...
			final List<BoogieType> indicesTypes = Arrays.stream(indices).map(exp -> (BoogieType) exp.getType())
					.collect(Collectors.toList());
			final BoogieType newType = TypeCheckHelper.typeCheckArrayAccessExpressionOrLhs(arrayType, indicesTypes,
					array.toString(), logger);
		...
		final BoogieType newType = TypeCheckHelper.typeCheckArrayAccessExpressionOrLhs(
				arrayType,
	...
	public ArrayLHS constructNestedArrayLHS(
			final ILocation loc,
			final LeftHandSide array,
			final Expression[] indices) {
		if (indices.length == 0) {
			throw new AssertionError("attempting to build array access without indices");
		}
		...
		final BoogieType lhsType = TypeCheckHelper.typeCheckArrayAccessExpressionOrLhs(
				arrayType,
				Arrays.asList(new BoogieType[] { (BoogieType) outerMostIndexValue.getType() }),
				array.toString(), logger);
	...
	public ArrayLHS constructNestedArrayLHS(final ILocation loc, final IBoogieType type, final LeftHandSide lhs,
			final Expression[] indices) {
		return constructNestedArrayLHS(loc, lhs, indices);
	}

	public IdentifierExpression constructIdentifierExpression(final ILocation loc, final BoogieType type,
	...
	public VariableLHS constructVariableLHS(final ILocation loc, final BoogieType type, final String identifier,
			final DeclarationInformation declarationInformation) {
		assert loc != null && type != null && identifier != null && declarationInformation != null;
		return new VariableLHS(loc, type, identifier, declarationInformation);
	}

	public ArrayStoreExpression constructArrayStoreExpression(
			final ILocation loc,
			final Expression array,
			final Expression[] indices,
		...
		final BoogieType type = TypeCheckHelper.typeCheckArrayStoreExpression(
				(BoogieType) array.getType(),
				indicesTypes,
				(BoogieType) value.getType(),
				array, logger);
		return new ArrayStoreExpression(loc, type, array, indices, value);
	}

	private BinaryExpression constructBinaryExpression(
			final ILocation loc,
			final Operator operator,
			final Expression operand1,
			final Expression operand2) {
		final String expr = operand1.toString() + operator.toString() + operand2.toString();
		final BoogieType type = TypeCheckHelper.typeCheckBinaryExpression(
				operator,
	...
	/**
	 * Construct a new {@link FunctionApplication} with the given C function
	 * identifier and the given Boogie arguments.
	 * This method
	 * <ul>
	 * <li>Renames the C function identifier to a
	 * {@link SupportedBitvectorOperations} identifier if it is appropriate.
	 ...
	 * </ul>
	 *
	 * If you only want to rename _this_ function application and assume that all
	 * arguments are already correct, then
	 * use
	 * {@link ExpressionFactory#constructFunctionApplication(ILocation, String, Expression[], BoogieType)}.
	 *
	 * @param loc
	 *                         A location
	 * @param identifier
	 ...
	 *                         the BoogieType of the result of the function
	 *                         application. This type must be provided because we
	 *                         cannot
	 *                         track all FunctionDeclarations for type checking, as
	 *                         they might not have been constructed yet.
	 * @return A Boogie function application.
	 */
	public Expression constructFunctionApplicationDeep(final ILocation loc, final String identifier,
	 ...
	 * @return A Boogie function application.
	 */
	public Expression constructFunctionApplication(final ILocation loc, final String identifier,
			final Expression[] arguments, final BoogieType resultBoogieType) {
		final String smtIdentifier = getBitvectorSmtFunctionNameFromCFunctionName(identifier);
		final SupportedBitvectorOperations sbo = getSupportedBitvectorOperation(smtIdentifier);
	...
	/**
	 * @return the matching {@link SupportedBitvectorOperations} value if the
	 *         identifier represents a supported
	 *         bitvector operation, or null if it does not.
	 */
	public SupportedBitvectorOperations getSupportedBitvectorOperation(final String identifier) {
		try {
	...
	public StructAccessExpression constructStructAccessExpression(
			final ILocation loc,
	...
	public BooleanLiteral createBooleanLiteral(final ILocation loc, final boolean value) {
		return new BooleanLiteral(loc, BoogieType.TYPE_BOOL, value);
	}

	public IntegerLiteral createIntegerLiteral(final ILocation loc, final String value) {
		return new IntegerLiteral(loc, BoogieType.TYPE_INT, value);
	}

	public BitvecLiteral createBitvecLiteral(final ILocation loc, final String value, final int length) {
		return new BitvecLiteral(loc, BoogieType.createBitvectorType(length), value, length);
	}

	public RealLiteral createRealLiteral(final ILocation loc, final String value) {
		return new RealLiteral(loc, BoogieType.TYPE_REAL, value);
	}

	public StringLiteral createStringLiteral(final ILocation loc, final String value) {
		// TODO: what boogie type should we give a string literal??
		return new StringLiteral(loc, value);
	}

	/**
	 * Returns an Expression that is the same as the given expression, except that
	 * its BoogieType has been changed to
	 * the given BoogieType.
	 *
	 * Note that this circumvents our type checks, thus should not be used for the
	 * final translation, as the type errors
	 * would occur in the BoogiePreprocessor.. (but is currently use in pre run
	 * mode).
	 *
	 * @param oe
	 * @param newType
	 * @return
	 */
	public Expression replaceBoogieType(final Expression oe, final BoogieType newType) {
		return modifyExpression(oe, x -> x, x -> newType);
	}

	public Expression modifyExpression(final Expression oe, final Function<ILocation, ILocation> oldToNewLoc,
	...
	public IdentifierExpression createVoidDummyExpression(final ILocation loc) {
		return constructIdentifierExpression(loc, BoogieType.TYPE_ERROR, DUMMY_VOID,
				DeclarationInformation.DECLARATIONINFO_GLOBAL);
	}

	public Expression constructBooleanWildCardExpression(final ILocation loc) {
		return new WildcardExpression(loc, BoogieType.TYPE_BOOL);
	}

	private FunctionApplication newFunctionApplication(final FunctionApplication node, final boolean isChanged,
	...
	/**
	 * Fills newArgs with either transformed or original parameters of the
	 * FunctionApplication.
	 *
	 * @return true iff some of the parameters changed during the transformation,
	 *         false otherwise
	 */
	private boolean handleFunctionApplicationParams(final FunctionApplication node,
	...
	/**
	 * Renames a function identifier by removing the first character and removing
	 * all occurrences of digits. This
	 * converts, e.g., ~bvadd32, to bvadd.
	 *
	 * @deprecated This is CTranslation-specific code that somehow survived the
	 *             transport in this plugin.
	 */
	@Deprecated
	private String getBitvectorSmtFunctionNameFromCFunctionName(final String name) {
		return name.substring(1).replaceAll("\\d+", "");
	}

	private Expression simplifyBitvectorExpression(final FunctionApplication node,
	...
	private Expression simplifyUnaryBitvectorExpression(final FunctionApplication node,
		...
		if (args[0] instanceof BitvecLiteral) {
			final BitvectorConstantOperationResult result = BitvectorConstant.apply(sbo,
					toConstant((BitvecLiteral) args[0]));
	...
	private Expression simplifyBinaryBitvectorExpression(final FunctionApplication node,
			final SupportedBitvectorOperations sbo, final Expression[] args) {
		...
		// TODO: We could still do things like (op (op c1 x) (op c2 y)) --> (op c3 op(x
		// y)) with c3 = (op c1 c2) for
		// certain ops
		if (left == null && right == null) {
			return node;
		}
	...
	private FunctionApplication simplifyBinaryAssociativeExpression(final FunctionApplication node,
		...
		if (right == null && args[1] instanceof FunctionApplication) {
			...
			final SupportedBitvectorOperations rightSbo = getSupportedBitvectorOperation(
					getBitvectorSmtFunctionNameFromCFunctionName(rightFunId));
			if (sbo == rightSbo) {
				final Expression innerLeft = rightFunApp.getArguments()[0];
				if (innerLeft instanceof BitvecLiteral) {
					final Expression newConst = computeBinaryBitvectorExpression(node, sbo, left,
	...
	private Expression computeBinaryBitvectorExpression(final FunctionApplication node,
	...
	private BitvectorConstant toConstant(final BitvecLiteral lit) {
		return new BitvectorConstant(new BigInteger(lit.getValue()), BigInteger.valueOf(lit.getLength()));
	}

	/**
	 * true iff left is annihilating (or absorbing) if it is the left operand of
	 * binOp, false otherwise
	 *
	 * If true, then (binOp left x) == left for any x
	 */
	private boolean isAnnihilatingLeft(final Operator binOp, final Expression left) {
		// TODO: Complete
		switch (binOp) {
			case ARITHMUL:
	...
	/**
	 * true iff right is annihilating (or absorbing) if it is the right operand of
	 * binOp, false otherwise
	 *
	 * If true, then (binOp right x) == right for any x
	 */
	private boolean isAnnihilatingRight(final Operator binOp, final Expression right) {
		// TODO: Complete
		switch (binOp) {
			case ARITHMUL:
			case ARITHPLUS:
	 ...
	 */
	private boolean isNeutralLeft(final Operator binOp, final Expression left) {
		// TODO: Complete
		switch (binOp) {
			case ARITHMUL:
				if (left instanceof IntegerLiteral) {
	...
	/**
	 * true iff right is neutral if it is the right operand of binOp, false
	 * otherwise
	 *
	 * If true, then (binOp right x) == x for any x
	 */
	private boolean isNeutralRight(final Operator binOp, final Expression right) {
		// TODO: Complete
		switch (binOp) {
			case ARITHMUL:
			case ARITHPLUS:
	 ...
	 */
	private boolean isCommutative(final Operator binOp) {
		// TODO: Complete
		switch (binOp) {
			case ARITHMUL:
			case ARITHPLUS:
	...
	/**
	 * true iff bConst is annihilating (or absorbing) if it is the left operand of
	 * op, false otherwise
	 *
	 * If true, then (op bConst x) == bConst for any x
	 */
	private boolean isAnnihilatingLeft(final SupportedBitvectorOperations op, final BitvectorConstant bConst) {
		switch (op) {
			case bvsdiv:
			case bvudiv:
			case bvsrem:
	...
	/**
	 * true iff bConst is annihilating (or absorbing) if it is the right operand of
	 * op, false otherwise
	 *
	 * If true, then (op x bConst) == bConst for any x
	 */
	private boolean isAnnihilatingRight(final SupportedBitvectorOperations op, final BitvectorConstant bConst) {
		switch (op) {
			case bvadd:
			case bvor:
			case bvmul:
	 ...
	 */
	private boolean isNeutralLeft(final SupportedBitvectorOperations op, final BitvectorConstant bConst) {
	 ...
	 */
	private boolean isNeutralRight(final SupportedBitvectorOperations op, final BitvectorConstant bConst) {
		switch (op) {
			case bvadd:
			case bvor:
			case bvmul:
	...
	private BitvecLiteral toBitvectorLiteral(final FunctionApplication node,
	...
	private BooleanLiteral toBooleanLiteral(final FunctionApplication node, final boolean value) {
		return new BooleanLiteral(node.getLoc(), node.getType(), value);
	}

	public Rational toRational(final String realLiteralValue) {
	...
	public Rational toRational(final BigInteger bigInt) {
		return Rational.valueOf(bigInt, BigInteger.ONE);
	}

	public Rational toRational(final BigDecimal bigDec) {
		Rational rat;
		if (bigDec.scale() <= 0) {
	...
	private final class BitvectorCFunctionNames2SmtFunctionNames extends GeneratedBoogieAstTransformer {
	...
	private final class BitvectorSmtFunctionNames2CFunctionNames extends GeneratedBoogieAstTransformer {
	...
	private final class SimplifyBitvectorExpression extends GeneratedBoogieAstTransformer {
