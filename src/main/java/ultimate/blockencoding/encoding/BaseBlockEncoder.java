...
public abstract class BaseBlockEncoder<LOC extends IcfgLocation> implements IEncoder<LOC> {
	...
	public BaseBlockEncoder(final BlockEncodingBacktranslator backtranslator, final Logger logger) {
		this.logger = logger;
		mRemovedEdges = 0;
		mRemovedLocations = 0;
	...
	protected void removeDisconnectedLocations(final BasicIcfg<LOC> root) {
		// check all locations without incoming edges that are not initial or which are
		// initial sinks
		final Set<IcfgLocation> initial = root.getProcedureEntryNodes().entrySet().stream().map(a -> a.getValue())
				.collect(Collectors.toSet());
		final Predicate<IcfgLocation> filter = a -> (!initial.contains(a) || a.getOutgoingEdges().isEmpty())
				&& a.getIncomingEdges().isEmpty();
		final Deque<IcfgLocation> locationsWithoutInEdge = root.getProgramPoints().entrySet().stream()
	...
	private static class DequeCollector<T> implements Collector<T, Deque<T>, Deque<T>> {
		...
		private static final Set<Collector.Characteristics> CHARACTERISTICS = EnumSet
