...
/**
 * Base class for encoder that try to minimize the nodes/states/locations of an
 * RCFG.
 *
...
public abstract class BaseMinimizeStates extends BaseBlockEncoder<IcfgLocation> {
	...
	public BaseMinimizeStates(final IcfgEdgeBuilder edgeBuilder, final BlockEncodingBacktranslator backtranslator,
			final BiPredicate<IIcfg<?>, IcfgLocation> funHasToBePreserved, final boolean ignoreBlowup,
	...
	protected final BasicIcfg<IcfgLocation> createResult(final BasicIcfg<IcfgLocation> icfg) {
		...
		while (!nodes.isEmpty()) {
			...
			closed.add(current);
			logger.fine("Processing node " + current + " [" + current.hashCode() + "]");
	...
	/**
	 * Process the state "target" and return a set of nodes that should be processed
	 * next. Processing means adding and
	 * removing edges. The caller will take care that this method is called only
	 * once per target node.
	 *
	 * @param icfg
	 *               The current ICFG.
	 * @param target
	 ...
	 *               here if you know that you do not need
	 *               to process them. You may also remove some nodes if you want to
	 *               re-process them.
	 * @return A set of nodes that should be processed. May not be null. Nodes
	 *         already in the closed set will not be
	 *         processed again.
	 */
	...
	private void checkForTimeoutOrCancellation() {
		if (!storage.getProgressMonitorService().continueProcessing()) {
			logger.info("Stopping block encoding due to timeout after removing " + mRemovedEdges + " edges and "
