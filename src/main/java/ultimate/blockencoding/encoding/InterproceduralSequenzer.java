...
public final class InterproceduralSequenzer extends BaseBlockEncoder<IcfgLocation> {
	...
	public InterproceduralSequenzer(final IcfgEdgeBuilder edgeBuilder, final BlockEncodingBacktranslator backtranslator,
			final Logger logger) {
	...
	protected BasicIcfg<IcfgLocation> createResult(final BasicIcfg<IcfgLocation> icfg) {
		...
		// find sequences of the form l --call--> l' --e--> l'' --return--> l''' and
		// merge them to one sequential
		// composition
		...
		while (!edges.isEmpty()) {
			...
			if (target == null) {
				// can happen if we see an edge that was disconnected during
				// createInterproceduralSequenceIfPossible but
		...
		if (isGraphStructureChanged()) {
			...
			logger.info("Removed " + mRemovedEdges + " edges and " + mRemovedLocations
	...
	private IcfgLocation createInterproceduralSequenceIfPossible(final IIcfgCallTransition<?> callTransition) {
		...
		final IcfgEdge intermediateCb = outCall.stream().findAny()
				.orElseThrow(() -> new AssertionError("Call with no out-edge"));
		...
		// TODO: We treated summaries different but in the IIcfg setting we cannot
		// differentiate them
		// final List<Summary> summaries =
		// callTransition.getSource().getOutgoingEdges().stream().filter(a -> a
		// instanceof Summary)
		...
		final IcfgEdge ss = createInterproceduralSequentialComposition(callTransition, intermediateCb,
				optionalReturn.get());
